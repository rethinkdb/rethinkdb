...
struct mysql_protocol_t : public protocol_t {
    ...
    virtual void connect(server_t *server) {
        ...
        if(use_db(false)) {
            ...
            const char range_read_stmt_str[] = "SELECT __value FROM bench WHERE __key BETWEEN ? AND ? LIMIT ?";
            res = mysql_stmt_prepare(range_read_stmt, range_read_stmt_str, strlen(range_read_stmt_str));
            if(res != 0) {
