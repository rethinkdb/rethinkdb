...
class dbench():
    ...
    def report_as_html(self):
        ...
        flot_data = 'data'
        for run, id in zip(self.bench_stats.bench_runs, range(len(self.bench_stats.bench_runs))):
            reduce(lambda x, y: x + y, run).json(self.out_dir + '/' + self.dir_str + '/' + flot_data + str(id))
            print >>res, '<p>',  flot('/' + self.prof_dir + '/' + self.dir_str + '/' + flot_data + str(id) + '.js', 'View data for run: %d' % id), '</p>'
