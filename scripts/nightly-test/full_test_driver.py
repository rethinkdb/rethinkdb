...
with simple_linear_db.LinearDBWriter("result_log.txt") as result_log:
    ...
    if not options.just_tests:
        ...
        if options.build_mode == "all" or options.build_mode == "minimal-with-os": # do the operating system builds
            ...
            builds["suse"] = {
                "kpommand_line": "cd rethinkdb/src && make rpm PACKAGING=1 PACKAGE_FOR_SUSE_10=1 SYMBOLS=1 DEBUG=1 UNIT_TESTS=0 LEGACY_LINUX=1 LEGACY_GCC=1 NO_EVENTFD=1 LEGACY_PROC_STAT=1 && mv ../build/packages ../build/suse",
