...
#    assert_raise(RuntimeError) {tbl.between(2, 1).run.to_a}
  ...
  def test_update_edge_cases
    assert_equal(data, tbl.order_by(:id).run.to_a)
    assert_equal({"unchanged" => 1}, tbl.get(0).update {nil}.run)
    assert_equal({"skipped" => 1}, tbl.get(11).update {nil}.run)
    assert_equal({"skipped" => 1}, tbl.get(11).update {{}}.run)
    assert_equal(data, tbl.order_by(:id).run.to_a)
  end
  ...
  def test_write
    ...
    assert_equal({'dropped' => 1},
                 r.db(db_name).table_drop((table_name + table_name)).run)
    assert_raise(RuntimeError) do
      r.db(db_name).table((table_name + table_name)).run.to_a
    end

    assert_equal({'dropped' => 1}, r.db_drop(db_name).run)
    ...
    assert_equal({"replaced" => 1}, tbl2.get(5).update {{:num => "5"}}.run)
    ...
    assert_equal({"replaced" => 1}, tbl2.get(0).replace {data[0]}.run)
    assert_equal(data, id_sort(tbl2.run.to_a))
    ...
    # insert
    assert_equal(len, tbl2.insert(r(data), :upsert).run["inserted"])
    ...
    assert_equal({"deleted" => 10, "inserted" => 10}, query.run)
    assert_equal(data, id_sort(tbl2.run.to_a))
