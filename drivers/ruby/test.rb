...
class ClientTest < Test::Unit::TestCase
  ...
  def test_ordering
    def order(query, *args); query.order_by(*args).run.to_a; end
  ...
  def test_easy_read # TABLE
    ...
    assert_equal(data, id_sort(r.db('test').table('tbl').run.to_a))
  end
  ...
  def test_map # MAP, FILTER, GETATTR, IMPLICIT_GETATTR, STREAMTOARRAY
    ...
    assert_equal(tbl.filter({'num' => 1}).run.to_a, [data[1]])
    query = tbl.order_by(:id).map { |outer_row|
  ...
  def test_reduce # REDUCE, HASATTR, IMPLICIT_HASATTR
    ...
    assert_equal(tbl.map{|r| r.contains(:id)}.run.to_a, data.map{true})
    assert_equal(tbl.map{|row| r.not row.contains('id')}.run.to_a, data.map{false})
    assert_equal(tbl.map{|row| r.not row.contains('id')}.run.to_a, data.map{false})
    assert_equal(tbl.map{|row| r.not row.contains('id')}.run.to_a, data.map{false})
    assert_equal(tbl.map{|r| r.contains(:id)}.run.to_a, data.map{true})
    assert_equal(tbl.map{|r| r.contains('id').not}.run.to_a, data.map{false})
  end
  ...
  def test_filter # FILTER
    ...
    assert_equal(id_sort(tbl.filter{|r| r[:id] < 5}.run.to_a), data[0...5])
  end
  ...
  def test_groupby
    assert_equal(tbl.order_by(:id).run.to_a, data)
    assert_equal(tbl.update{|row| {:gb => row[:id] % 3}}.run,
    ...
    assert_equal(tbl.replace{|row| row.unpick(:gb)}.run,
  ...
  def test_key_generation
    ...
    assert_equal(tbl.get(res['generated_keys'][0]).delete.run['deleted'], 1)
    assert_equal(tbl.filter{|row| (row[:id] < 0) | (row[:id] > 1000)}.delete.run['deleted'], 2)
    assert_equal(tbl.order_by(:id).run.to_a, data)
  end
  ...
  def test_order_by # ORDERBY, MAP
    assert_equal(tbl.order_by(:id).run.to_a, data)
    assert_equal(tbl.order_by('id').run.to_a, data)
    assert_equal(tbl.order_by(:id).run.to_a, data)
    assert_equal(tbl.order_by(r.asc(:id)).run.to_a, data)
    assert_equal(tbl.order_by([:id, false]).run.to_a, data.reverse)
    assert_equal(tbl.order_by(r.desc(:id)).run.to_a, data.reverse)
    ...
    assert_equal(tbl.order_by(:id).run.to_a, tbl.order_by(r.asc(:id)).run.to_a)
  end

  def test_concatmap # CONCATMAP, DISTINCT
    assert_equal(r([1,2,3]).concat_map{r([1,2]).to_stream}.to_stream.run.to_a,
                 [1,2,1,2,1,2])
    assert_equal(r([[1],[2]]).concat_map{|x| x}.run.to_a, [1,2])
  ...
  def test_range # RANGE
    ...
    assert_equal(id_sort(tbl.between(1,3).run.to_a), data[1..3])
    assert_equal(id_sort(tbl.between(3,3).run.to_a), data[3..3])
    assert_equal(id_sort(tbl.between(2,nil).run.to_a), data[2..-1])
    assert_equal(id_sort(tbl.between(1, 3).run.to_a), data[1..3])
    assert_equal(id_sort(tbl.between(nil, 4).run.to_a), data[0..4])

    assert_equal(id_sort(tbl.to_array.between(1,3).run.to_a), data[1..3])
    assert_equal(id_sort(tbl.to_array.between(2,nil).run.to_a), data[2..-1])
    assert_equal(id_sort(tbl.to_array.between(1, 3).run.to_a), data[1..3])
    assert_equal(id_sort(tbl.to_array.between(nil, 4).run.to_a), data[0..4])
  ...
  def test_groupedmapreduce # GROUPEDMAPREDUCE
    ...
    assert_equal(tbl.order_by(:id).run.to_a, data)
    assert_equal(tbl.to_array.order_by(:id).run.to_a, data)
    assert_equal(r([{:id => 1}, {:id => 0}]).order_by(:id).run.to_a.map{|x| x['id']}, [0, 1])
    ...
    gmr = tbl.grouped_map_reduce(lambda {|row| row[:id] % 4}, lambda {|row| row[:id]},
                               0, lambda {|a,b| a+b});
    gmr2 = tbl.grouped_map_reduce(lambda {|r| r[:id] % 4}, lambda {|r| r[:id]},
                                0, lambda {|a,b| a+b});
    gmr3 = tbl.to_array.grouped_map_reduce(lambda {|row| row[:id] % 4},
                                         lambda {|row| row[:id]},
                                         0, lambda {|a,b| a+b});
  ...
  def test_nth # NTH
    assert_equal(tbl.order_by(:id).nth(2).run, data[2])
  end
  ...
  def test_javascript_vars # JAVASCRIPT
    ...
    assert_equal(id_sort(tbl.map{|x| r.js("#{x}")}.run.to_a), id_sort(tbl.run.to_a))
    assert_equal(id_sort(tbl.map{ r.js("this")}.run.to_a), id_sort(tbl.run.to_a))
    assert_equal(tbl.map{|x| r.js("#{x}.num")}.run.to_a.sort, tbl.map{|r| r[:num]}.run.to_a.sort)
    assert_equal(tbl.map{ r.js("this.num")}.run.to_a.sort, tbl.map{|r| r[:num]}.run.to_a.sort)
    assert_equal(id_sort(tbl.filter{|x| r.js("#{x}.id < 5")}.run.to_a),
                 id_sort(tbl.filter{|r| r[:id] < 5}.run.to_a))
    assert_equal(id_sort(tbl.filter{r.js("this.id < 5")}.run.to_a),
                 id_sort(tbl.filter{|r| r[:id] < 5}.run.to_a))
  end
  ...
  def test_pointdelete
    assert_equal(tbl.order_by(:id).run.to_a, data)
    assert_equal(tbl.get(0).delete.run, {'deleted' => 1})
    assert_equal(tbl.get(0).delete.run, {'deleted' => 0})
    assert_equal(tbl.order_by(:id).run.to_a, data[1..-1])
    assert_equal(tbl.insert(data[0], :upsert).run,
                 {'inserted' => 1, 'errors' => 0})
    assert_equal(tbl.order_by(:id).run.to_a, data)
  end
  ...
  def test_write_python # from python tests
    ...
    docs = [{"a" => 3, "b" => 10, "id" => 1},
            {"a" => 9, "b" => -5, "id" => 2},
            {"a" => 9, "b" => 3, "id" => 3}]
    assert_equal(tbl.insert(docs, :upsert).run,
                 {'inserted' => docs.length, 'errors' => 0})
    docs.each {|doc| assert_equal(tbl.get(doc['id']).run, doc)}
    assert_equal(tbl.distinct(:a).run.to_a.sort, [3,9].sort)
    assert_equal(tbl.filter({'a' => 3}).run.to_a, [docs[0]])
    assert_raise(RuntimeError){tbl.filter({'a' => tbl.count + ""}).run.to_a}
    assert_not_nil(tbl.insert({'a' => 3}, :upsert).run['first_error'])

    assert_equal(tbl.get(0).run, nil)

    assert_equal(tbl.insert({:id => 100, :text => "グルメ"}, :upsert).run,
                 {'inserted' => 1, 'errors' => 0})
    assert_equal(tbl.get(100)[:text].run, "グルメ")

    tbl.delete.run
    docs = [{"id" => 1}, {"id" => 2}]
    assert_equal(tbl.insert(docs, :upsert).run,
                 {'inserted' => docs.length, 'errors' => 0})
    assert_equal(tbl.limit(1).delete.run, {'deleted' => 1})
    assert_equal(tbl.run.to_a.length, 1)

    tbl.delete.run
    docs = (0...4).map{|n| {"id" => 100 + n, "a" => n, "b" => n % 3}}
    assert_equal(tbl.insert(docs, :upsert).run,
                 {'inserted' => docs.length, 'errors' => 0})
    assert_equal(tbl.insert(r(docs).to_stream, :upsert).run,
                 {'inserted' => docs.length, 'errors' => 0})
    docs.each{|doc| assert_equal(tbl.get(doc['id']).run, doc)}
    ...
    assert_equal(tbl.insert(docs, :upsert).run,
                 {'inserted' => docs.length, 'errors' => 0})
    def filt(expr, fn)
      assert_equal(tbl.filter(exp).order_by(:id).run.to_a, docs.select{|x| fn x})
    end

    # TODO: still an error?
    # assert_raise(ArgumentError){r[:a] == 5}
    # assert_raise(ArgumentError){r[:a] != 5}
    assert_equal(id_sort(tbl.filter{|r| r[:a] < 5}.run.to_a), docs.select{|x| x['a'] < 5})
    assert_equal(id_sort(tbl.filter{|r| r[:a] <= 5}.run.to_a), docs.select{|x| x['a'] <= 5})
    assert_equal(id_sort(tbl.filter{|r| r[:a] > 5}.run.to_a), docs.select{|x| x['a'] > 5})
    assert_equal(id_sort(tbl.filter{|r| r[:a] >= 5}.run.to_a), docs.select{|x| x['a'] >= 5})
    assert_equal(id_sort(tbl.filter{|r| r[:a].eq(r[:b])}.run.to_a),
    ...
    assert_equal(tbl.insert(docs, :upsert).run,
                 {'inserted' => docs.length, 'errors' => 0})
    assert_equal(tbl.replace{|x| x}.run,
                 {'modified' => docs.length, 'inserted' => 0,
                   'deleted' => 0, 'errors' => 0, 'errors' => 0})
    assert_equal(id_sort(tbl.run.to_a), docs)
    assert_equal(tbl.update{nil}.run, {'updated' => 0, 'skipped' => 10, 'errors' => 0})
  ...
  def test_array_foreach #FOREACH
    ...
    assert_equal(r([2,3,4]).for_each{|x|
                   [tbl.get(x).update{{:num => 0}},
                    tbl.get(x*2).update{{:num => 0}}]}.run,
                 {"skipped"=>0, "updated"=>6, "errors"=>0})
    assert_equal(tbl.to_array.for_each{|row|
                   tbl.filter{|r| r[:id].eq(row[:id])}.update{|row|
  ...
  def test_foreach_multi #FOREACH
    ...
    assert_equal(tbl.insert(data, :upsert).run,
                 {'inserted' => 10, 'errors' => 0})
    assert_equal(tbl2.insert(data, :upsert).run,
                 {'inserted' => 10, 'errors' => 0})

    query = tbl.for_each {|row|
      tbl2.for_each {|row2|
    ...
    assert_equal(tbl.for_each{|row| tbl4.insert(row, :upsert)}.run,
                 {'inserted' => 10, 'errors' => 0})
    assert_equal(tbl2.for_each{|row| tbl4.filter{|r| r[:id].eq(row[:id])}.delete}.run,
                 {'deleted' => 6})
  ...
  def test_fancy_foreach #FOREACH
    ...
    query = tbl.order_by(:id).for_each { |row|
      [tbl.update { |row2|
    ...
    assert_equal(tbl.order_by(:id).run.to_a,
                 [{"num"=>4,  "id"=>2, "name"=>"2"},
                  {"num"=>8,  "id"=>4, "name"=>"4"},
                  {"num"=>12, "id"=>6, "name"=>"6"},
                  {"num"=>16, "id"=>8, "name"=>"8"}])
  ...
  def test_big_between
    data_100 = (0...100).map{|x| {:id => x}}
    assert_equal(tbl.insert(data_100, :upsert).run,
                 {'inserted' => 100, 'errors' => 0})
    assert_equal(id_sort(tbl.between(1, 2).run.to_a), [{'id' => 1}, {'id' => 2}])

    assert_equal(tbl.delete.run, {'deleted' => 100})
    assert_equal(tbl.insert(data, :upsert).run,
                 {'inserted' => 10, 'errors' => 0})
    assert_equal(id_sort(tbl.run.to_a), data)
  end
  ...
  def test_mutate_edge_cases #POINTMUTATE
    ...
    assert_equal(res['errors'], 10)
    assert_equal(tbl.order_by(:id).run.to_a, data)
    assert_equal(tbl.get(0).replace{nil}.run,
    ...
    assert_equal(tbl.get(0).replace{data[0]}.run,
    ...
    assert_equal(tbl.get(0).replace{|row| r.branch(row.eq(nil), data[1], data[0])}.run,
  ...
  def test_insert
    ...
    assert_equal(tbl.order_by(:id).run.to_a, data2)
    assert_equal(tbl.delete.run, {'deleted' => data2.length})
  ...
  def test_update_edge_cases #POINTUPDATE
    assert_equal(tbl.order_by(:id).run.to_a, data)
    assert_equal(tbl.get(0).update{nil}.run,
                 {'skipped' => 1, 'updated' => 0, 'errors' => 0})
    assert_equal(tbl.get(11).update{nil}.run,
                 {'skipped' => 1, 'updated' => 0, 'errors' => 0})
    assert_equal(tbl.get(11).update{{}}.run,
                 {'skipped' => 1, 'updated' => 0, 'errors' => 0})
    assert_equal(tbl.order_by(:id).run.to_a, data)
  end
  ...
  def test_write
    ...
    assert_equal(tbl2.insert(data, :upsert).run['inserted'], len)
    assert_equal(tbl2.insert(data + data, :upsert).run['inserted'], len*2)
    assert_equal(id_sort(tbl2.run.to_a), data)
    assert_equal(tbl2.insert({:id => 0, :broken => true, 'errors' => 0}, :upsert).run['inserted'], 1)
    assert_equal(tbl2.insert({:id => 1, :broken => true, 'errors' => 0}, :upsert).run['inserted'], 1)
    assert_equal(id_sort(tbl2.run.to_a)[2..len-1], data[2..len-1])
    assert_equal(id_sort(tbl2.run.to_a)[0]['broken'], true)
    assert_equal(id_sort(tbl2.run.to_a)[1]['broken'], true)
    ...
    mutate = tbl2.replace{|x| {:id => x[:id], :num => x[:num], :name => x[:name]}}.run
    assert_equal(mutate, {"modified"=>10, 'inserted' => 0, "deleted"=>0, 'errors' => 0, 'errors' => 0})
    assert_equal(id_sort(tbl2.run.to_a), data)

    mutate = tbl2.replace{|row| r.branch(row[:id].eq(4) | row[:id].eq(5),
                                    {:id => -1}, row)}.run
    ...
    assert_equal(tbl2.get(5).update{{:num => "5"}}.run,
    ...
    assert_equal(tbl2.get(5).run['num'], '5')
    assert_equal(tbl2.replace{|row|
    ...
    assert_equal(tbl2.get(0).run['num'], 2);
    update = tbl2.get(0).update{nil}.run
    assert_equal(update, {'errors' => 0, 'updated' => 0, 'skipped' => 1})
    assert_equal(tbl2.get(0).replace{data[0]}.run,
                 {"errors"=>0, 'inserted' => 0, "deleted"=>0, "modified"=>1, 'errors' => 0})
    assert_equal(id_sort(tbl2.run.to_a), data)

    # DELETE
    assert_equal(tbl2.filter{|r| r[:id] < 5}.delete.run['deleted'], 5)
    assert_equal(id_sort(tbl2.run.to_a), data[5..-1])
    assert_equal(tbl2.delete.run['deleted'], len-5)
    assert_equal(tbl2.run.to_a, [])

    # insert
    assert_equal(tbl2.insert(r(data).to_stream, :upsert).run['inserted'], len)
    tbl2.insert(data, :upsert).run
    assert_equal(id_sort(tbl2.run.to_a), data)

    # MUTATE
    assert_equal(tbl2.replace{|row| row}.run,
                 {'modified' => len, 'inserted' => 0, 'deleted' => 0, 'errors' => 0, 'errors' => 0})
    assert_equal(id_sort(tbl2.run.to_a), data)
    assert_equal(tbl2.replace{|row| r.branch(row[:id] < 5, nil, row)}.run,
                 {'modified' => 5, 'inserted' => 0, 'deleted' => len-5, 'errors' => 0, 'errors' => 0})
    assert_equal(id_sort(tbl2.run.to_a), data[5..-1])
    assert_equal(tbl2.insert(data[0...5], :upsert).run,
                 {'inserted' => 5, 'errors' => 0})

    # FOREACH, POINTDELETE
    # TODO_SERV: Return value of foreach should change (Issue #874)
    query = tbl2.for_each{|row| [tbl2.get(row[:id]).delete, tbl2.insert(row, :upsert)]}
    assert_equal(query.run,
    ...
    tbl2.insert(data, :upsert).run
    assert_equal(id_sort(tbl2.run), data)
    ...
    assert_equal(tbl2.get(0).replace{nil}.run,
                 {'modified' => 0, 'inserted' => 0, 'deleted' => 1, 'errors' => 0, 'errors' => 0})
    assert_equal(id_sort(tbl2.run.to_a), data[1..-1])
    assert_raise(RuntimeError){tbl2.get(1).replace{{:id => -1}}.run.to_a}
    assert_raise(RuntimeError){tbl2.get(1).replace{|row| {:name => row[:name]*2}}.run.to_a}
    assert_equal(tbl2.get(1).replace{|row| row.merge({:num => 2})}.run,
                 {'modified' => 1, 'inserted' => 0, 'deleted' => 0, 'errors' => 0, 'errors' => 0})
    assert_equal(tbl2.get(1).run['num'], 2);
    assert_equal(tbl2.insert(data[0...2], :upsert).run,
                 {'inserted' => 2, 'errors' => 0})
