...
class ToTerm
  ...
  def method_missing(meth, *args, &block)
    ...
    if enum_type(Builtin::BuiltinType, meth)
      _ [:call, [meth], args]
    elsif enum_type(Builtin::Comparison, meth)
      _ [:call, [:compare, meth], args]
    else
      _ [meth, *args]
    end
  end
...
class SexpTerm
  ...
  def method_missing(meth, *args, &block)
    ...
    elsif enum_type(Builtin::BuiltinType, meth) and not message_field(Builtin, meth)
      _ [:call, [meth], [@body, *args]]
    else
      _ [:call, [meth, *args], [@body]]
    end
  end
end
