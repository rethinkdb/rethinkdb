...
class Renderer(object):
    ...
    def __init__(self, template_dir, invoking_filename):
        self.template_dir = template_dir
        self.invoking_filename = invoking_filename
    ...
    def render(self,
        ...
        if self.already_rendered(tpl, output_path, dependencies):
            print("Already rendered:", output_path)
            return
    ...
    def dependent_templates(self, tpl):
        ...
        deps = dependencies.union([tpl.filename])
        self.dependency_cache[tpl.filename] = deps
        return deps

    def already_rendered(self, tpl, output_path, dependencies):
        '''Check if rendered file is already up to date'''
        if not os.path.exists(output_path):
            # print(" - rendering since", output_path, "doesn't exist")
            return False
        ...
        for dep in dependencies:
            if self.mtime(dep) > output_mtime:
                # print(" - rendering since", dep, "is newer")
                return False
        return True

    @staticmethod
    def autogenerated_header(template_path, output_path, filename):
        rel_tpl = os.path.relpath(template_path, start=output_path)

        return ('// Autogenerated by {}.\n'
                '// Do not edit this file directly.\n'
                '// The template for this file is located at:\n'
                '// {}\n').format(os.path.basename(filename), rel_tpl)
