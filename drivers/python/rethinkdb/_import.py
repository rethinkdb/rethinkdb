...
from ._compat import range_, dict_keys, PY2
...
def import_from_queue(progress, conn, task_queue, error_queue, use_upsert, durability):
    if progress[0] is None:
        ...
        for i in reversed(range_(len(task[2]))):
...
def csv_reader(task_queue, filename, db, table, options, progress_info, exit_event):
    ...
    with open(filename, "r") as file_in:
        ...
        for row in reader:
            ...
            for key in list(dict_keys(obj)): # Treat empty fields as no entry rather than empty string
                if len(obj[key]) == 0:
                    del obj[key]
...
def spawn_import_clients(options, files_info):
    ...
    try:
        ...
        for i in range_(options["clients"]):
...
def import_directory(options):
    ...
    for (root, dirs, files) in os.walk(options["directory"]):
        if not dbs:
            ...
            dbs = True
            if len(db_filter) > 0:
                for i in reversed(range_(len(dirs))):
