...
def compare(expected, result, options=None):
    ...
    if options['explicit_type'] and not isinstance(result, options['explicit_type']):
        return FalseStr(
            'expected explicit type %s, got %s (%s)' % (options['explicit_type'], result, type(result).__name__))
    ...
    if isinstance(expected, (Err, Exception)):
        ...
        if isinstance(expected, Err):
            if not isinstance(result, expected.err_type):
                return FalseStr(
                    'expected error type %s, got %r (%s)' % (expected.err_type, result, type(result).__name__))
        elif not isinstance(result, type(expected)):
            return FalseStr(
                'expected error type %s, got %r (%s)' % (type(expected).__name__, result, type(result).__name__))
    ...
    # -- Regex/UUID
    if isinstance(expected, (Regex, RE_PATTERN_FIELD)):
    ...
    # -- type
    if not isinstance(expected, type(result)) and (type(expected) != type(object) or not issubclass(expected, type(
            result))):  # reversed to so we can handle subclasses
...
class PyTestDriver(object):
    ...
    def define(self, expr, variable):
        ...
        try:
            exec(compile('%s = %s' % (variable, expr), '<string>', 'single'),
                 self.scope)  # handle things like: a['b'] = b
        except Exception as e:
...
def setup_table(table_variable_name, table_name, db_name='test'):
    ...
    def _clean_table(table_name, db_name):
        ...
        r.db(db_name).table(table_name).index_list().for_each(r.db(db_name).table(table_name).index_drop(r.row)).run(
            driver.connection())
...
def setup_table_check():
    ...
    if len(required_external_tables) > 0:
        raise Exception(
            'Unused external tables, that is probably not supported by this test: %s' % ('%s.%s' % tuple(x) for x in
                                                                                         required_external_tables).join(
                ', '))
...
def float_cmp(expected_value):
    if not isinstance(expected_value, float):
        raise ValueError(
            'value must be of type `float` but got: %r (%s)' % (expected_value, type(expected_value).__name__))
