        ...
        if info['envName'] in os.environ:
            if os.environ[info['envName']].lower() == '--installed--':
                ...
                if not os.path.exists(target):
                    warnings.warn('Supplied %s driver path (%s) was not valid: %s' % (
                        name, info['envName'], os.environ[info['envName']]))
                    continue
...
def find_rethinkdb_executable(mode=None):
    result_path = os.environ.get('RDB_EXE_PATH') or os.path.join(latest_build_dir(check_executable=True, mode=mode),
                                                                 'rethinkdb')

    if not os.access(result_path, os.X_OK):
        raise test_exceptions.NotBuiltException(
            detail='The rethinkdb server executable is not available: %s' % str(result_path))
...
def latest_build_dir(check_executable=True, mode=None):
    ...
    if candidatePath is None or (
            check_executable is True and not os.access(os.path.join(candidatePath, 'rethinkdb'), os.X_OK)):
        raise test_exceptions.NotBuiltException(
            detail='the rethinkdb server executable was not present/runnable in: %s' % candidatePath)
...
def import_python_driver():
    ...
    if sourcePath:
        try:
            ...
            raise test_exceptions.NotBuiltException(detail='Failed making Python driver from: %s' % sourcePath,
    ...
    if not loadedDriver.__file__.startswith(driverPath):
        raise ImportError(
            'Loaded Python driver was %s, rather than the expected one from %s' % (loadedDriver.__file__, driverPath))
...
class RunningProcesses:
    class Process:
        ...
        def __str__(self):
            return 'pid: %d ppid: %d pgid: %d status: %r command: %s' % (
                self.pid, self.ppid, self.pgid, self.status, self.command)
    ...
    def list(self):
        ...
        while candidates or candidateGroups:
            for candidate in candidates.copy():
                ...
                if candidate.status and candidate.status[0] in (
                        'D', 'I', 'R', 'S') and candidate not in targetProcesses:
...
def getShardRanges(conn, table, db='test'):
    ...
    splitPointsRaw = \
        conn._r.db('rethinkdb').table('_debug_table_status').get(conn._r.db(dbName).table(tableName).config()['id'])[
            'shard_scheme']['split_points'].run(conn)
