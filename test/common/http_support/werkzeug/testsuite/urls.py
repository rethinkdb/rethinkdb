...
class URLsTestCase(WerkzeugTestCase):
    ...
    def test_quoting(self):
        self.assert_strict_equal(urls.url_quote('\xf6\xe4\xfc'), '%C3%B6%C3%A4%C3%BC')
        self.assert_strict_equal(urls.url_unquote(urls.url_quote('#%="\xf6')), '#%="\xf6')
        ...
        self.assert_strict_equal(urls.url_unquote_plus('foo%2Bbar'), 'foo+bar')
        self.assert_strict_equal(urls.url_encode({b'a': None, b'b': b'foo bar'}), 'b=foo+bar')
        self.assert_strict_equal(urls.url_encode({'a': None, 'b': 'foo bar'}), 'b=foo+bar')
        self.assert_strict_equal(urls.url_fix('http://de.wikipedia.org/wiki/Elf (Begriffsklärung)'),
    ...
    def test_bytes_unquoting(self):
        self.assert_strict_equal(urls.url_unquote(urls.url_quote(
            '#%="\xf6', charset='latin1'), charset=None), b'#%="\xf6')

    def test_url_decoding(self):
        x = urls.url_decode(b'foo=42&bar=23&uni=H%C3%A4nsel')
        self.assert_strict_equal(x['foo'], '42')
        self.assert_strict_equal(x['bar'], '23')
        self.assert_strict_equal(x['uni'], 'Hänsel')
        ...
        self.assert_strict_equal(x['Üh'], 'Hänsel')
    ...
    def test_url_encoding(self):
        ...
        self.assert_strict_equal(urls.url_encode(d, sort=True, separator=';'), 'bar=23;blah=H%C3%A4nsel;foo=1')

    def test_sorted_url_encode(self):
        self.assert_strict_equal(urls.url_encode({"a": 42, "b": 23, 1: 1, 2: 2},
            sort=True, key=lambda i: text_type(i[0])), '1=1&2=2&a=42&b=23')
        self.assert_strict_equal(urls.url_encode({'A': 1, 'a': 2, 'B': 3, 'b': 4}, sort=True,
    ...
    def test_iri_support(self):
        ...
        self.assert_strict_equal(urls.iri_to_uri('http://☃.net/'), 'http://xn--n3h.net/')
        self.assert_strict_equal(
        ...
        # idna codec
        self.assert_strict_equal(urls.uri_to_iri(b'/foo'), '/foo')
        self.assert_strict_equal(urls.iri_to_uri('/foo'), '/foo')

        self.assert_strict_equal(urls.iri_to_uri('http://föö.com:8080/bam/baz'),
                          'http://xn--f-1gaa.com:8080/bam/baz')

    def test_iri_safe_conversion(self):
        self.assert_strict_equal(urls.iri_to_uri('magnet:?foo=bar'),
                                 'magnet:?foo=bar')
        self.assert_strict_equal(urls.iri_to_uri('itms-service://?foo=bar'),
    ...
    def test_href(self):
        ...
        self.assert_strict_equal(x('foo'), 'http://www.example.com/foo')
        self.assert_strict_equal(x.foo('bar'), 'http://www.example.com/foo/bar')
        self.assert_strict_equal(x.foo('bar', x=42), 'http://www.example.com/foo/bar?x=42')
        self.assert_strict_equal(x.foo('bar', class_=42), 'http://www.example.com/foo/bar?class=42')
        self.assert_strict_equal(x.foo('bar', {'class': 42}), 'http://www.example.com/foo/bar?class=42')
        self.assert_raises(AttributeError, lambda: x.__blah__)

        x = urls.Href('blah')
        self.assert_strict_equal(x.foo('bar'), 'blah/foo/bar')
    ...
    def test_href_url_join(self):
        x = urls.Href('test')
        self.assert_line_equal(x('foo:bar'), 'test/foo:bar')
        self.assert_line_equal(x('http://example.com/'), 'test/http://example.com/')
    ...
    def test_url_unquote_plus_unicode(self):
        # was broken in 0.6
        self.assert_strict_equal(urls.url_unquote_plus('\x6d'), '\x6d')
        self.assert_is(type(urls.url_unquote_plus('\x6d')), text_type)
    ...
    def test_url_attributes(self):
        ...
        self.assert_strict_equal(rv.host, '\N{SNOWMAN}.com')
    ...
    def test_partial_unencoded_decode(self):
        ...
        self.assert_strict_equal(x['foo'], '정상처리')
