...
public:
    ...
    bool operator==(const dummy_raft_state_t &other) {
...
public:
    ...
    dummy_raft_cluster_t(
        ...
        for (const raft_member_id_t &member_id : initial_config.voting_members) {
            add_member(
                member_id,
                raft_persistent_state_t<raft_dummy_state_t, uuid_u>::make_initial(
    ...
    raft_member_id_t join() {
        ...
        add_member(
            member_id,
            raft_persistent_state_t<raft_dummy_state_t, uuid_u>::make_join());
        return member_id;
    }

    /* `set_live()` puts the given member into the given state. */
    void set_live(const raft_member_id_t &member_id, live_t live) {
        member_info_t *i = &member_info.at(member_id);
        if (i->drainer.has() && live != live_t::alive) {
            alive_members.apply_atomic_op([&](std::set<raft_member_id_t> *members) {
...
private:
    class member_info_t : public raft_network_and_storage_interface_t {
    public:
        bool send_request_vote_rpc(
            ...
            do_rpc(dest, [&](dummy_raft_member_t *other, signal_t *interruptor2) {
                    other->member->on_request_vote_rpc(
        ...
        bool send_install_snapshot_rpc(
                ...
                raft_term_t last_included_term, const state_t &snapshot_state,
                const raft_complex_config_t &snapshot_configuration,
                signal_t *interruptor,
                raft_term_t *term_out) {
            do_rpc(dest, [&](dummy_raft_member_t *other, signal_t *interruptor2) {
                    other->member->on_install_snapshot_rpc(
        ...
        bool send_append_entries_rpc(
                const raft_member_id_t &dest, raft_term_t term,
                const raft_member_id_t &leader_id, const raft_log_t<change_t> &entries,
                raft_log_index_t leader_commit,
                signal_t *interruptor,
                raft_term_t *term_out, bool *success_out) {
            do_rpc(dest, [&](dummy_raft_member_t *other, signal_t *interruptor2) {
                    other->member->on_append_entries_rpc(
        ...
        clone_ptr_t<watchable_t<std::set<raft_member_id_t> > > get_connected_members() {
            return alive_members.get_watchable();
        }
        ...
        bool do_rpc(
                ...
                signal_t *interruptor) {
            block(interruptor);
            member_info_t *other = &parent->members.at(dest);
        ...
        void block(signal_t *interruptor) {
            ...
            if (randint(10) == 0) {
                signal_timer_t timer(randint(30));
                timer.start();
    ...
    void check_invariants(UNUSED auto_drainer_t::lock_t keepalive) {
        ...
        for (const auto &pair : members) {
            if (pair.second.member.has()) {
                ...
                members.insert(pair.second.member.get());
            }
        }
        dummy_raft_member_t::check_invariants(members);
    }

    std::map<raft_member_id_t, member_info_t> members;
...
public:
    dummy_raft_traffic_generator_t(dummy_raft_cluster_t *cluster, int ms) :
        cluster(_cluster),
        timer(ms, [this]() {
