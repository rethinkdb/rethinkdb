...
void run_single_metadata_test() {
    ...
    connectivity_cluster_t::run_t cr(&c, port, &slm, 0, NULL);
...
void run_metadata_exchange_test() {
    ...
    connectivity_cluster_t::run_t run1(&cluster1, port, &slm1, 0, NULL);
    connectivity_cluster_t::run_t run2(&cluster2, port+1, &slm2, 0, NULL);
...
void run_watcher_test() {
    ...
    connectivity_cluster_t::run_t run(&cluster, port, &slm, 0, NULL);
