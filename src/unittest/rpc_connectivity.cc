...
void run_start_stop_test() {
    ...
    connectivity_cluster_t::run_t cr1(&c1, get_unittest_addresses(), port, NULL, portno_t::zero(), NULL);
    connectivity_cluster_t::run_t cr2(&c2, get_unittest_addresses(), port.with_offset(1), NULL, portno_t::zero(), NULL);
    connectivity_cluster_t::run_t cr3(&c3, get_unittest_addresses(), port.with_offset(2), NULL, portno_t::zero(), NULL);
...
void run_message_test() {
    ...
    connectivity_cluster_t::run_t cr1(&c1, get_unittest_addresses(), port, &a1, portno_t::zero(), NULL);
    connectivity_cluster_t::run_t cr2(&c2, get_unittest_addresses(), port.with_offset(1), &a2, portno_t::zero(), NULL);
    connectivity_cluster_t::run_t cr3(&c3, get_unittest_addresses(), port.with_offset(2), &a3, portno_t::zero(), NULL);
    cr2.join(c1.get_peer_address(c1.get_me()));
    cr3.join(c1.get_peer_address(c1.get_me()));
...
void run_get_peers_list_test() {
...

    {
        ...
        EXPECT_EQ(port.with_offset(1), c1.get_peer_address(c2.get_me()).port);
...
void run_event_watchers_test() {
    ...
    connectivity_cluster_t c2;
    scoped_ptr_t<connectivity_cluster_t::run_t> cr2(new connectivity_cluster_t::run_t(&c2, get_unittest_addresses(), port.with_offset(1), NULL, portno_t::zero(), NULL));
...
void run_stop_mid_join_test() {
    ...
    for (int i = 0; i < num_members; i++) {
        ...
        runs[i].init(new connectivity_cluster_t::run_t(nodes[i].get(), get_unittest_addresses(), port.with_offset(i), NULL, portno_t::zero(), NULL));
    }
...
void run_multiplexer_test() {
    ...
    connectivity_cluster_t::run_t c1r(&c1, get_unittest_addresses(), port, &c1mr, portno_t::zero(), NULL);
    connectivity_cluster_t::run_t c2r(&c2, get_unittest_addresses(), port.with_offset(1), &c2mr, portno_t::zero(), NULL);
...
void run_check_headers_test() {
    ...
    tcp_conn_stream_t conn(addr.primary_ip(), addr.port, &cond, portno_t::zero());
