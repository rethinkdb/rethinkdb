...
public:
    ...
    test_range_map_t(int l, int r, char v) :
        inner(l, r, make_copy(v)), ref_offset(l), ref(r - l, v)
    {
        validate();
    }
    int left_edge() const {
    ...
    void extend_right(int l, int r, char v) {
        inner.extend_right(l, r, make_copy(v));
        while (l++ < r) {
            ref.push_back(v);
        }
        validate();
    ...
    void extend_left(int l, int r, char v) {
        inner.extend_left(l, r, make_copy(v));
        while (r-- > l) {
            ref.push_front(v);
        }
        ref_offset = l;
    ...
    for (int repeat = 0; repeat < 1000; ++repeat) {
        ...
        if (opcode == 0) {
            ...
            test = test.map_multi(l, r, [&](int l2, int r2, char v) {
                range_map_t<int, char> chunk(l2, r2, make_copy(v));
