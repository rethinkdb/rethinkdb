...
namespace unittest {
    ...
    TEST(JSON, SizeEstimate) {
        struct cJSON *obj = cJSON_Parse("{}");
        ASSERT_TRUE(obj);
        ASSERT_EQ(cJSON_estimate_size(obj), sizeof(struct cJSON));
        cJSON_Delete(obj);
        ...
        obj = cJSON_Parse("false");
        ASSERT_TRUE(obj);
        ASSERT_EQ(cJSON_estimate_size(obj), sizeof(struct cJSON));
        cJSON_Delete(obj);

        obj = cJSON_Parse("null");
        ASSERT_TRUE(obj);
        ASSERT_EQ(cJSON_estimate_size(obj), sizeof(struct cJSON));
        cJSON_Delete(obj);
        ...
        obj = cJSON_Parse("\"str\"");
        ASSERT_TRUE(obj);
        ASSERT_EQ(cJSON_estimate_size(obj), sizeof(struct cJSON) + 3);
        cJSON_Delete(obj);
        ...
        obj = cJSON_Parse("[1,2,3]");
        ASSERT_TRUE(obj);
        ASSERT_EQ(cJSON_estimate_size(obj), 4*sizeof(struct cJSON));
        cJSON_Delete(obj);

        obj = cJSON_Parse("{\"a\": 1, \"b\": 2}");
        ASSERT_TRUE(obj);
        ASSERT_EQ(cJSON_estimate_size(obj), 3*sizeof(struct cJSON) + 2);
        cJSON_Delete(obj);

        obj = cJSON_Parse("[\"abc\", \"def\"]");
        ASSERT_TRUE(obj);
        ASSERT_EQ(cJSON_estimate_size(obj), 3*sizeof(struct cJSON) + 6);
        cJSON_Delete(obj);

        obj = cJSON_Parse("{\"a\": {\"cde\": \"str\"}, \"b\": [0.5, null] }");
        ASSERT_TRUE(obj);
        ASSERT_EQ(cJSON_estimate_size(obj), 6*sizeof(struct cJSON) + 8);
        cJSON_Delete(obj);
    }
} //namespace unittest
