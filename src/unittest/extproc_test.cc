...
public:
    ...
    uint64_t run() {
        write_message_t wm;
        serialize<cluster_version_t::LATEST_VERSION>(&wm, iterations);
        {
        ...
        archive_result_t res
            = deserialize<cluster_version_t::LATEST_VERSION>(extproc_job.read_stream(),
                                                             &result);
...
private:
    static bool worker_fn(read_stream_t *stream_in, write_stream_t *stream_out) {
        ...
        {
            archive_result_t res
                = deserialize<cluster_version_t::LATEST_VERSION>(stream_in, &count);
            guarantee(res == archive_result_t::SUCCESS);
        }

        uint64_t result = fib(count);
        write_message_t wm;
        serialize<cluster_version_t::LATEST_VERSION>(&wm, result);
...
public:
    ...
    collatz_job_t(uint64_t n, extproc_pool_t *pool, signal_t *interruptor) :
        ...
        write_message_t wm;
        serialize<cluster_version_t::LATEST_VERSION>(&wm, last_value);
        int res = send_write_message(extproc_job.write_stream(), &wm);
        guarantee(res == 0);
    }
    ...
    ~collatz_job_t() {
        ...
        write_message_t wm;
        serialize<cluster_version_t::LATEST_VERSION>(&wm, 'q');
        int res = send_write_message(extproc_job.write_stream(), &wm);
        guarantee(res == 0);
    }
    ...
    uint64_t step() {
        ...
        write_message_t wm;
        serialize<cluster_version_t::LATEST_VERSION>(&wm, 'c');
        int res = send_write_message(extproc_job.write_stream(), &wm);
        guarantee(res == 0);
        return last_value;
    }
...
private:
    static bool worker_fn(read_stream_t *stream_in, write_stream_t *stream_out) {
        ...
        {
            archive_result_t res
                = deserialize<cluster_version_t::LATEST_VERSION>(stream_in,
                                                                 &current_value);
            guarantee(res == archive_result_t::SUCCESS);
        }
        ...
        do {
            ...
            // Send current value.
            write_message_t wm;
            serialize<cluster_version_t::LATEST_VERSION>(&wm, current_value);
            {
                int res = send_write_message(stream_out, &wm);
                guarantee(res == 0);
            }
            ...
            archive_result_t res
                = deserialize<cluster_version_t::LATEST_VERSION>(stream_in, &command);
            guarantee(res == archive_result_t::SUCCESS);
        } while (command == 'c');
...
public:
    ...
    void write() {
        write_message_t wm;
        serialize<cluster_version_t::LATEST_VERSION>(&wm, 100);
        int res = send_write_message(extproc_job.write_stream(), &wm);
        if (res != 0) {
            throw std::runtime_error("read failed");
        }
    ...
    static bool worker_fn(read_stream_t *stream_in, write_stream_t *) {
        while (true) {
            ...
            UNUSED archive_result_t res
                = deserialize<cluster_version_t::LATEST_VERSION>(stream_in, &data);
        }
...
public:
    ...
    void corrupt(bool direction) {
        write_message_t wm;
        serialize<cluster_version_t::LATEST_VERSION>(&wm, direction);
        int res = send_write_message(extproc_job.write_stream(), &wm);
        guarantee(res == 0);
...
private:
    ...
    static bool worker_fn(read_stream_t *stream_in, write_stream_t *stream_out) {
        ...
        {
            archive_result_t res
                = deserialize<cluster_version_t::LATEST_VERSION>(stream_in, &send_data);
            guarantee(res == archive_result_t::SUCCESS);
        }

        if (send_data) {
            write_message_t wm;
            serialize<cluster_version_t::LATEST_VERSION>(&wm, send_data);
            int res = send_write_message(stream_out, &wm);
            guarantee(res == 0);
        }
