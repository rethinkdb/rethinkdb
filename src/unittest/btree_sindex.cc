...
void run_sindex_reset_data_test() {
    temp_file_t temp_file;
    ...
    for (int i = 0; i < 50; ++i) {
...

        {
            ...
            boost::shared_ptr<scoped_cJSON_t> data(
                    new scoped_cJSON_t(cJSON_CreateNumber(1)));
    ...
    unsigned int blocks_before = store.cache->num_blocks();
    {
        //Now we reset the data
        write_token_pair_t token_pair;
        store.new_write_token_pair(&token_pair);

        store.reset_data(rdb_protocol_t::region_t::universe(),
                         btree_store_t<rdb_protocol_t>::metainfo_t(),
                         &token_pair,
                         &dummy_interuptor);
    }

    ASSERT_LT(store.cache->num_blocks(), blocks_before);
}

TEST(BTreeSindex, BtreeStoreAPIResetData) {
    run_in_thread_pool(&run_sindex_reset_data_test);
}
