    ...
    for (int i = 0; i < 2000; ++i) {
        ...
        if (perform_index_write) {
            // Do an index write creating the block.
            {
                ...
                write_ops.push_back(index_write_op_t(block_id, optional(tokens[0]), optional(repli_timestamp_t::distant_past)));
            ...
            {
                std::vector<index_write_op_t> write_ops;
                write_ops.push_back(index_write_op_t(block_id, optional(counted_t<standard_block_token_t>())));
