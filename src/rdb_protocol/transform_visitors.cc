...
public:
    transform_visitor_t(std::shared_ptr<const scoped_cJSON_t> _json,
                        std::list<std::shared_ptr<const scoped_cJSON_t> > *_out,
                        ql::env_t *_ql_env,
...
transform_visitor_t::transform_visitor_t(std::shared_ptr<const scoped_cJSON_t> _json,
                                         json_list_t *_out,
                                         ql::env_t *_ql_env,
                                         const backtrace_t &_backtrace)
...
void transform_visitor_t::operator()(ql::map_wire_func_t &func) const {  // NOLINT(runtime/references)
    counted_t<const ql::datum_t> arg(new ql::datum_t(json));
