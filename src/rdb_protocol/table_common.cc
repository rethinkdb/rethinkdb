...
counted_t<const ql::datum_t> make_row_replacement_stats(
    ...
    if (old_row->get_type() == ql::datum_t::R_NULL) {
        ...
        started_empty = false;
#ifndef NDEBUG
        counted_t<const ql::datum_t> old_row_pval = old_row->get(primary_key_name);
    ...
    if (new_row->get_type() == ql::datum_t::R_NULL) {
        ...
        new_row->rcheck_valid_replace(
            old_row, counted_t<const ql::datum_t>(), primary_key_name);
        counted_t<const ql::datum_t> new_primary_key_value =
            new_row->get(primary_key_name, ql::NOTHROW);
        rcheck_target(new_row, ql::base_exc_t::GENERIC,
            ...
            (started_empty
             ? strprintf("Primary key `%s` cannot be changed (null -> %s)",
                         primary_key_name.c_str(), new_row->print().c_str())
    ...
    *was_changed_out = *old_row != *new_row;
    ...
    if (started_empty) {
        ...
        if (ended_empty) {
            ...
            r_sanity_check(
                *old_row->get(primary_key_name) == *new_row->get(primary_key_name));
            if (!*was_changed_out) {
...
counted_t<const ql::datum_t> make_row_replacement_error_stats(
        counted_t<const ql::datum_t> old_row,
        return_changes_t return_changes,
...
counted_t<const ql::datum_t> resolve_insert_conflict(
        const std::string &primary_key,
        counted_t<const ql::datum_t> old_row,
        counted_t<const ql::datum_t> insert_row,
