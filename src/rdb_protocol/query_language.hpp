...
void wait_for_rdb_table_readiness(namespace_repo_t<rdb_protocol_t> *ns_repo, namespace_id_t namespace_id, signal_t *interruptor, const std::shared_ptr<semilattice_readwrite_view_t<cluster_semilattice_metadata_t> > &semilattice_metadata) THROWS_ONLY(interrupted_exc_t);
