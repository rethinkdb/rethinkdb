...
struct backfill_atom_t {
    store_key_t key;
    std::shared_ptr<const scoped_cJSON_t> value;
    repli_timestamp_t recency;

    backfill_atom_t() { }
    backfill_atom_t(const store_key_t& _key,
                    const std::shared_ptr<const scoped_cJSON_t>& _value,
...
struct rget_item_t {
    rget_item_t() { }
    rget_item_t(const store_key_t &_key, std::shared_ptr<const scoped_cJSON_t> _data)
        : key(_key), data(_data) { }

    rget_item_t(const store_key_t &_key, std::shared_ptr<const scoped_cJSON_t> _sindex_key,
                std::shared_ptr<const scoped_cJSON_t> _data)
        : key(_key), sindex_key(_sindex_key), data(_data) { }

    store_key_t key;
    boost::optional<std::shared_ptr<const scoped_cJSON_t> > sindex_key;
...
struct rdb_protocol_t {
    ...
    struct rget_read_response_t {
        ...
        typedef std::map<std::shared_ptr<const scoped_cJSON_t>, std::shared_ptr<const scoped_cJSON_t>, shared_scoped_less_t> groups_t; // Present if the terminal was a groupedmapreduce
        typedef std::shared_ptr<const scoped_cJSON_t> atom_t; // Present if the terminal was a reduction
    ...
    public:
        point_write_t() { }
        point_write_t(const store_key_t& _key, std::shared_ptr<const scoped_cJSON_t> _data, bool _overwrite = true)
