...
private:
    virtual counted_t<val_t> eval_impl(UNUSED eval_flags_t flags) {
        ...
        counted_t<func_t> g = arg(1)->as_func(env, PLUCK_SHORTCUT);
        counted_t<func_t> m = arg(2)->as_func(env);
        counted_t<func_t> r = arg(3)->as_func(env);
        return new_val(arg(0)->as_seq(env)->gmr(env, g, m, base, r));
    }
