...
return_changes_t parse_return_changes(
    eval_error *err_out, scope_env_t *env, args_t *args, backtrace_id_t bt) {
    auto v_return_vals = args->optarg(err_out, env, "return_vals");
    if (err_out->has()) { return return_changes_t::NO /* irrelevant value */; }
    if (v_return_vals) {
        rfail_src(bt, base_exc_t::LOGIC,
                  "Error: encountered obsolete optarg `return_vals`.  "
                  "Use `return_changes` instead.");
    }
    scoped_ptr_t<val_t> v = args->optarg(err_out, env, "return_changes");
    if (err_out->has()) { return return_changes_t::NO /* irrelevant value */; }
    if (v) {
...
private:
    static void maybe_generate_key(counted_t<table_t> tbl,
        ...
        if (!(*datum_out).get_field_nothrow(datum_string_t(tbl->get_pkey())).has()) {
    ...
    eval_impl(eval_error *err_out, scope_env_t *env, args_t *args, eval_flags_t) const {
        ...
        return_changes_t return_changes = parse_return_changes(err_out, env, args, backtrace());
        if (err_out->has()) { return noval(); }

        optional<counted_t<const ql::func_t> > conflict_func;

        scoped_ptr_t<val_t> conflict_optarg = args->optarg(err_out, env, "conflict");
        if (err_out->has()) { return noval(); }
        const conflict_behavior_t conflict_behavior
            = parse_conflict_optarg(args->optarg(err_out, env, "conflict"));
        if (err_out->has()) { return noval(); }
        const durability_requirement_t durability_requirement
            = parse_durability_optarg(args->optarg(err_out, env, "durability"));
        if (err_out->has()) { return noval(); }

        scoped_ptr_t<val_t> ignore_write_hook_arg =
            args->optarg(err_out, env, "ignore_write_hook");
        if (err_out->has()) { return noval(); }
...
private:
    virtual scoped_ptr_t<val_t> eval_impl(
        ...
        auto non_atomic_optarg = args->optarg(err_out, env, "non_atomic");
        if (err_out->has()) { return noval(); }
        if (non_atomic_optarg) {
            nondet_ok = non_atomic_optarg->as_bool();
        }
        return_changes_t return_changes = parse_return_changes(err_out, env, args, backtrace());
        if (err_out->has()) { return noval(); }
            ...
            for (;;) {
                ...
                if (!f->is_deterministic().test(single_server_t::no, constant_now_t::yes)) {
                    ...
                    for (const auto &val : vals) {
                        datum_t field = val.get_field_with_err(err_out, datum_string_t(tbl->get_pkey()));
                        if (err_out->has()) { return noval(); }
...
private:
    ...
    eval_impl(eval_error *err_out, scope_env_t *env, args_t *args, eval_flags_t) const {
        ...
        auto v0 = args->arg(err_out, env, 0);
        if (err_out->has()) { return noval(); }
        counted_t<datum_stream_t> ds = v0->as_seq(env->env);
