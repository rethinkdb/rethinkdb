    ...
    eval_impl(scope_env_t *env, args_t *args, eval_flags_t) const {
        ...
        /* Add a sorting to the table if we're doing indexed sorting. */
            ...
            rcheck(tbl_slice.has(), base_exc_t::LOGIC,
                   "Indexed order_by can only be performed on a TABLE or TABLE_SLICE, not on a `NAME_OF_TYPE_ORDER_BY_WAS_CALLED_ON`.  Make sure order_by comes before any transformations (such as map) or filters.");
