...
private:
    virtual counted_t<val_t> eval_impl(scope_env_t *env, UNUSED eval_flags_t flags) {
        return new_val(env, arg(env, 0)->as_seq(env)->map(env->env, arg(env, 1)->as_func(env)));
    }
...
private:
    virtual counted_t<val_t> eval_impl(scope_env_t *env, UNUSED eval_flags_t flags) {
        return new_val(env, arg(env, 0)->as_seq(env)->concatmap(env->env, arg(env, 1)->as_func(env)));
    }
...
private:
    virtual counted_t<val_t> eval_impl(scope_env_t *env, UNUSED eval_flags_t flags) {
        ...
        if (v0->get_type().is_convertible(val_t::type_t::SELECTION)) {
            ...
            return new_val(env, v0->as_seq(env)->filter(env->env, f, default_filter_val));
        }
    }
...
private:
    virtual counted_t<val_t> eval_impl(scope_env_t *env, UNUSED eval_flags_t flags) {
        return new_val(arg(env, 0)->as_seq(env)->reduce(env->env,
                                                        optarg(env, "base"),
