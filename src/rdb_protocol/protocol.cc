...
struct r_shard_visitor : public boost::static_visitor<read_t> {
    ...
    read_t operator()(const point_read_t &pr) const {
        rassert_reviewed(rdb_protocol_t::monokey_region(pr.key) == region);
        return read_t(pr);
    }

    read_t operator()(const rget_read_t &rg) const {
        rassert_reviewed(region_is_superset(rg.region, region));
        rget_read_t _rg(rg);
        _rg.region = region;
        return read_t(_rg);
    }
    ...
    read_t operator()(const distribution_read_t &dg) const {
        rassert_reviewed(region_is_superset(dg.region, region));
...
public:
    ...
    void operator()(const point_read_t &) {
        ...
        guarantee_reviewed(boost::get<point_read_response_t>(&responses[0].response));
...
struct w_shard_visitor : public boost::static_visitor<write_t> {
    ...
    write_t operator()(const point_write_t &pw) const {
        rassert_reviewed(rdb_protocol_t::monokey_region(pw.key) == region);
        return write_t(pw);
    }
    ...
    write_t operator()(const point_delete_t &pd) const {
        rassert_reviewed(rdb_protocol_t::monokey_region(pd.key) == region);
        return write_t(pd);
    }
    const region_t &region;
};
