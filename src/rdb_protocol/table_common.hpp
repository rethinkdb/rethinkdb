...
/* `make_row_replacement_stats()` performs two jobs: it checks that a change to a row is
legal, and it generates a stats object describing that change. If the change is illegal,
it throws a QL exception. */
...
row in the case where an exception was thrown. If `make_row_replacement_stats()` throws
...
/* `perform_insert` computes what the new value of a row should be after performing an
insert operation with the given conflict behavior. It may throw a QL exception. */
