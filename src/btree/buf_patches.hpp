...
private:
    template <class V>
    friend void leaf_patched_insert(value_sizer_t<V> *sizer, buf_t *node, const btree_key_t *key, const void *value, repli_timestamp_t tstamp);
...
private:
    friend void leaf_patched_remove(buf_t *node, const btree_key_t *key, repli_timestamp_t tstamp);
...
private:
    friend void leaf_patched_erase_presence(buf_t *node, const btree_key_t *key);
