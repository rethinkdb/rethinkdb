...
public:
    ...
    get_result_t get(const store_key_t &key, order_token_t token, const boost::scoped_ptr<transaction_t>& txn, got_superblock_t& superblock);
    ...
    mutation_result_t change(const mutation_t &m, castime_t castime, order_token_t token, boost::scoped_ptr<transaction_t>& txn, got_superblock_t& superblock);
    ...
    // if key_range is NULL, the whole thing is backfilled
