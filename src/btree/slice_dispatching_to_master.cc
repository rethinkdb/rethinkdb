...
get_result_t btree_slice_dispatching_to_master_t::get_cas(store_key_t *key, castime_t maybe_castime) {
    on_thread_t th(slice_->home_thread);
...
set_result_t btree_slice_dispatching_to_master_t::sarc(store_key_t *key, data_provider_t *data, mcflags_t flags, exptime_t exptime, castime_t maybe_castime, add_policy_t add_policy, replace_policy_t replace_policy, cas_t old_cas) {
    on_thread_t th(slice_->home_thread);
    castime_t castime = generate_if_necessary(maybe_castime);
...
incr_decr_result_t btree_slice_dispatching_to_master_t::incr_decr(incr_decr_kind_t kind, store_key_t *key, uint64_t amount, castime_t maybe_castime) {
    on_thread_t th(slice_->home_thread);
    castime_t castime = generate_if_necessary(maybe_castime);
...
append_prepend_result_t btree_slice_dispatching_to_master_t::append_prepend(append_prepend_kind_t kind, store_key_t *key, data_provider_t *data, castime_t maybe_castime) {
    on_thread_t th(slice_->home_thread);
    castime_t castime = generate_if_necessary(maybe_castime);
    if (master_) {
        buffer_borrowing_data_provider_t borrower(master_->home_thread, data);
...
delete_result_t btree_slice_dispatching_to_master_t::delete_key(store_key_t *key, repli_timestamp maybe_timestamp) {
    on_thread_t th(slice_->home_thread);
