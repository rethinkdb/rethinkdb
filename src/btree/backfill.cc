...
struct backfill_traversal_helper_t : public btree_traversal_helper_t, public home_thread_mixin_t {
    // FIXME: why population_change_out is not used?
    void process_a_leaf(transaction_t *txn, buf_lock_t *leaf_node_buf, const btree_key_t *left_exclusive_or_null, const btree_key_t *right_inclusive_or_null, int * /*population_change_out*/) {
        assert_thread();
...
void do_agnostic_btree_backfill(value_sizer_t<void> *sizer, btree_slice_t *slice, const key_range_t& key_range, repli_timestamp_t since_when,
                                agnostic_backfill_callback_t *callback, transaction_t *txn, got_superblock_t& superblock, traversal_progress_t *p) {
    rassert(coro_t::self());
