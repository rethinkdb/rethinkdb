...
template <class V>
void strprint_entry(std::string& out, value_sizer_t<V> *sizer, const entry_t *entry) {
    if (entry_is_live(entry)) {
        ...
        out += strprintf("[value size=%d]", sizer->size(entry_value<V>(entry)));
...
template <class V>
void strprint(std::string& out, value_sizer_t<V> *sizer, const leaf_node_t *node) {
...
template <class V>
void print(FILE *fp, value_sizer_t<V> *sizer, const leaf_node_t *node) {
...
template <class V>
void init(value_sizer_t<V> *sizer, leaf_node_t *node) {
...
template <class V>
int mandatory_cost(value_sizer_t<V> *sizer, const leaf_node_t *node, int required_timestamps, int *tstamp_back_offset_out) {
...
template <class V>
int mandatory_cost(value_sizer_t<V> *sizer, const leaf_node_t *node, int required_timestamps) {
    int ignored;
    return mandatory_cost(sizer, node, required_timestamps, &ignored);
}

template <class V>
int leaf_epsilon(value_sizer_t<V> *sizer) {
...
template <class V>
bool is_full(value_sizer_t<V> *sizer, const leaf_node_t *node, const btree_key_t *key, const void *value) {
...
template <class V>
bool is_underfull(value_sizer_t<V> *sizer, const leaf_node_t *node) {
...
template <class V>
void garbage_collect(value_sizer_t<V> *sizer, leaf_node_t *node, int num_tstamped) {
    int ignore = 0;
...
template <class V>
void split(value_sizer_t<V> *sizer, leaf_node_t *node, leaf_node_t *rnode, btree_key_t *median_out) {
...
template <class V>
bool level(value_sizer_t<V> *sizer, leaf_node_t *node, leaf_node_t *sibling, btree_key_t *key_to_replace_out, btree_key_t *replacement_key_out) {
...
template <class V>
bool is_mergable(value_sizer_t<V> *sizer, const leaf_node_t *node, const leaf_node_t *sibling) {
...
public:
    ...
    template <class V>
    const V *get_value(const leaf_node_t *node) const {
