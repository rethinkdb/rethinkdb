...
protected:
    ...
    // given offset.
    static int get_timestamp_offset(block_size_t block_size, btree_leaf_node *node, uint16_t offset);
};
...
class leaf_key_comp {
    ...
    btree_key *key;
    public:
    explicit leaf_key_comp(const btree_leaf_node *_node) : node(_node), key(NULL)  { }
