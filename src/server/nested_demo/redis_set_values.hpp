...
public:
    ...
    bool sismember(value_sizer_t<redis_demo_set_value_t> *super_sizer, boost::shared_ptr<transaction_t> transaction, const std::string &member) const;

    int scard() const;

    // returns true if a member has been deleted (i.e. existed in the set)
    bool srem(value_sizer_t<redis_demo_set_value_t> *super_sizer, boost::shared_ptr<transaction_t> transaction, const std::string &member);

    // Returns true iff the member was new
    bool sadd(value_sizer_t<redis_demo_set_value_t> *super_sizer, boost::shared_ptr<transaction_t> transaction, const std::string &member);
    ...
    /*
     ...
     must not simultaneously hold a lock on both redis_demo_set_value_t keys in the
     ...
     you can create the destination redis_demo_set_value_t without actually inserting
...
public:
    value_sizer_t<redis_demo_set_value_t>(block_size_t bs) : block_size_(bs) { }

    int size(const redis_demo_set_value_t *value) const {
        return value->inline_size(block_size_);
    }

    bool fits(UNUSED const redis_demo_set_value_t *value, UNUSED int length_available) const {
    ...
    int max_possible_size() const {
        // It's of constant size...
        return sizeof(redis_demo_set_value_t);
    }
