...
get_result_t btree_slice_dispatching_to_masterstore_t::get_cas(store_key_t *key, castime_t castime) {
...
set_result_t btree_slice_dispatching_to_masterstore_t::sarc(store_key_t *key, data_provider_t *data, mcflags_t flags, exptime_t exptime, castime_t castime, add_policy_t add_policy, replace_policy_t replace_policy, cas_t old_cas) {
    on_thread_t th(slice_->home_thread);
...
incr_decr_result_t btree_slice_dispatching_to_masterstore_t::incr_decr(incr_decr_kind_t kind, store_key_t *key, uint64_t amount, castime_t castime) {
    on_thread_t th(slice_->home_thread);
...
append_prepend_result_t btree_slice_dispatching_to_masterstore_t::append_prepend(append_prepend_kind_t kind, store_key_t *key, data_provider_t *data, castime_t castime) {
    on_thread_t th(slice_->home_thread);
    if (masterstore_) {
        buffer_borrowing_data_provider_t borrower(masterstore_->home_thread, data);
...
delete_result_t btree_slice_dispatching_to_masterstore_t::delete_key(store_key_t *key, repli_timestamp timestamp) {
    on_thread_t th(slice_->home_thread);
