...
struct counted2_node_t {
    block_magic_t magic;
} __attribute__((__packed__));

struct internal_counted2_node_t : counted2_node_t {
...
struct counted2_value_t {
    float score;
    char blb[0];

    int size(block_size_t &blksize) const {
        blob_t b(const_cast<char *>(blb), blob::btree_maxreflen);
        return sizeof(counted2_value_t) + b.refsize(blksize);
    }
} __attribute__((__packed__));

struct leaf_counted2_node_t : counted2_node_t {
    // The number of blob references stored in this leaf
    uint16_t n_refs;
    char refs[0]; // refs are to variable sized counted2_value_t's
...
struct counted_btree2_t {
    ...
    counted_btree2_t(sub_ref2_t *root_, block_size_t blocksize, transaction_t *transaction) :
        root(root_), blksize(blocksize), txn(transaction) {;}

    counted_btree2_t(block_size_t blocksize) : blksize(blocksize) {;}
    ...
    private:
        ...
        const counted2_value_t *current_val;
...
protected:
    const counted2_value_t *at_recur(buf_lock_t &buf, unsigned index);
    bool insert_recur(buf_lock_t &blk, float score, std::string &value,
            block_id_t *new_blk_out, unsigned *new_size_out, float *split_score_out);
    bool internal_insert(buf_lock_t &blk, float score, std::string &value,
            block_id_t *new_blk_out, unsigned *new_size_out, float *split_score_out);
