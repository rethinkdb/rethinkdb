...
T *tracking_allocator_t<T, Allocator>::allocate(size_t n) {
...
template <class T, class Allocator>
T *tracking_allocator_t<T, Allocator>::allocate(size_t n, const void *hint) {
    if (auto p = parent.lock()) {
        if (!(p->debit(sizeof(T) * n))) {
            rfail_toplevel(ql::base_exc_t::GENERIC,
                           "Failed to allocate memory; exceeded allocation threshold");
...
template <class T, class Allocator>
void tracking_allocator_t<T, Allocator>::deallocate(T *region, size_t n) {
    if (auto p = parent.lock()) p->credit(sizeof(T) * n);
    this->original.deallocate(region, n);
}

template <class T, class Allocator>
size_t tracking_allocator_t<T, Allocator>::max_size() const {
