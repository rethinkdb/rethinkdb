...
void do_rget(txt_memcached_handler_t *rh, pipeliner_t *pipeliner, order_source_t *order_source, int argc, char **argv) {
    ...
    try {
        ...
        /* Make a list of (real, non-hashed) shards to search */
        std::set<key_range_t> real_shards;
        for (std::set<hash_region_t<key_range_t> >::const_iterator shard_it = shards.begin();
             shard_it != shards.end(); ++shard_it) {
            if (range.overlaps(shard_it->inner)) {
                real_shards.insert(shard_it->inner);
            }
        }

        std::set<key_range_t>::const_iterator shard_it = real_shards.begin();

        while (max_items > 0) {
            rget_query_t rget_query(region_intersection(memcached_protocol_t::region_t(range), memcached_protocol_t::region_t(*shard_it)), max_items);
            ...
            if (results.truncated) {
                ...
                ++shard_it;
                if (shard_it == real_shards.end() || !range.overlaps(*shard_it)) {
...
void do_storage(txt_memcached_handler_t *rh, pipeliner_t *pipeliner, storage_command_t sc, int argc, char **argv, order_token_t token) {
    ...
    to `std::bind()` */
    // RSI: is this no longer necessary?
    storage_metadata_t metadata(mcflags, exptime, unique);

    pipeliner_acq->done_argparsing();

    coro_t::spawn_now_dangerously(std::bind(&run_storage_command, rh, pipeliner_acq.release(), sc, key, dp, metadata, noreply, token));
}
