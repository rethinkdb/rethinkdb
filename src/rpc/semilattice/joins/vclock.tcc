...
void vclock_t<T>::cull_old_values() {
    guarantee_reviewed(!values.empty(), "As a precondition, values should never be empty\n");
    ...
    guarantee_reviewed(!values.empty(), "As a postcondition, values should never be empty\n");
}
...
bool vclock_t<T>::in_conflict() const {
    guarantee_reviewed(!values.empty());
