...
directory_write_manager_t<metadata_t>::directory_write_manager_t(
        ...
        cluster_manager_t::message_tag_t message_tag_,
    ...
    connections_change_subscription([this] () { on_connections_change(); })
{
    ...
    typename watchable_t<cluster_manager_t::connection_map_t>::freeze_t connections_freeze(cluster_manager->get_connections());
    guarantee(cluster_manager->get_connections()->get().empty());
    value_change_subscription.reset(value, &value_freeze);
    connections_change_subscription.reset(cluster_manager->get_connections(), &connections_freeze);
}
...
void directory_write_manager_t<metadata_t>::on_connections_change() THROWS_NOTHING {
    ...
    cluster_manager_t::connection_map_t current_connections = cluster_manager->get_connections()->get();
        ...
        if (last_connections.count(connection) == 0) {
            ...
            coro_t::spawn_sometime(
                [=] () {
                    ...
                    cluster_manager->send_message(connection, connection_keepalive, message_tag, &writer);
                });
        }
    }
