...
public:
    printf_buffer_t();
    explicit printf_buffer_t(const char *format, ...) ATTRIBUTE_FORMAT(printf, 2, 3);
    printf_buffer_t(va_list ap, const char *format) ATTRIBUTE_FORMAT(printf, 3, 0);
    ~printf_buffer_t();

    // append and vappend become slow if the total size grows to N or
    // greater.  We still have std::vector-style amortized constant
    // time growth, though.
    void appendf(const char *format, ...) ATTRIBUTE_FORMAT(printf, 2, 3);
    void vappendf(const char *format, va_list ap) ATTRIBUTE_FORMAT(printf, 2, 0);
