...
transaction_t *co_begin_transaction(cache_t *cache, access_t access, int expected_change_count) {
    cache->ensure_thread();
    transaction_begun_callback_t cb;
    transaction_t *value = cache->begin_transaction(access, expected_change_count, &cb);
    if (!value) {
        value = cb.join();
    }
    rassert(value);
