...
template<class inner_cache_t>
bool scc_cache_t<inner_cache_t>::offer_read_ahead_buf(block_id_t block_id, uint32_t this_block_size,
                                                      void *buf,
                                                      const counted_t<standard_block_token_t>& token,
                                                      repli_timestamp_t recency_timestamp) {
    return inner_cache.offer_read_ahead_buf(block_id, this_block_size, buf, token,
                                            recency_timestamp);
}
