...
void writeback_t::local_buf_t::set_dirty(bool _dirty) {
    ...
    if (!dirty && _dirty) {
        ...
        throttling_acq.init(new throttling_semaphore_acq_t(
...
class writeback_t::buf_writer_t :
    ...
    bool io_completed_;
    scoped_ptr_t<throttling_semaphore_acq_t> throttling_acq;
...
public:
    ...
    buf_writer_t(writeback_t *wb, scoped_ptr_t<mc_buf_lock_t> &&buf, scoped_ptr_t<throttling_semaphore_acq_t> &&t_acq)
        : io_completed_(false) {
