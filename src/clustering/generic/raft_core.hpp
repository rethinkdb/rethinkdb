...
/* Every member of the Raft cluster is identified by a `server_id_t`. The Raft paper
uses integers for this purpose, but we use UUIDs because we have no reliable distributed
way of assigning integers, but we've already assigned a `server_id_t` to each server in
the cluster. */
...
public:
    ...
    */
    std::set<server_id_t> voting_members, non_voting_members;

    /* Returns a list of all members, voting and non-voting. */
    std::set<server_id_t> get_all_members() const {
        std::set<server_id_t> members;
    ...
    bool is_quorum(const std::set<server_id_t> &members) const {
        size_t votes = 0;
        for (const server_id_t &m : members) {
    ...
    bool is_valid_leader(const server_id_t &member) const {
...
public:
    ...
    std::set<server_id_t> get_all_members() const {
        std::set<server_id_t> members = config.get_all_members();
        if (is_joint_consensus()) {
            /* Raft paper, Section 6: "Log entries are replicated to all servers in both
            configurations." */
            std::set<server_id_t> members2 = new_config->get_all_members();
...
public:
    ...
    likely to succeed. */
    virtual clone_ptr_t<watchable_t<std::set<server_id_t> > >
...
public:
    raft_member_t(
        const server_id_t &this_member_id,
...
private:
    ...
    */
    ...
    void leader_update_match_index(
        ...
        std::map<server_id_t, raft_log_index_t> *match_index,
        server_id_t key,
    ...
    */
    ...
    void leader_spawn_update_coros(
        ...
        std::map<server_id_t, raft_log_index_t> *match_indexes,
        /* A map containing an `auto_drainer_t` for each running update coroutine. */
        std::map<server_id_t, scoped_ptr_t<auto_drainer_t> > *update_drainers,
        const new_mutex_acq_t *mutex_acq);
    ...
    server_id_t current_term_leader_id;
