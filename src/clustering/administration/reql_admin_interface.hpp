...
/* `cluster_reql_admin_interface_t` is a concrete subclass of `reql_admin_interface_t` that translates the user's
`table_create()`, `table_drop()`, etc. requests into specific actions on the semilattices. By performing these actions
through the abstract `reql_admin_interface_t`, we can keep the ReQL code separate from the semilattice code. */

class cluster_reql_admin_interface_t : public reql_admin_interface_t {
public:
    cluster_reql_admin_interface_t(
            uuid_u my_machine_id,
            boost::shared_ptr<semilattice_readwrite_view_t<cluster_semilattice_metadata_t> > semilattices,
...
private:
    ...
    void wait_for_metadata_to_propagate(const cluster_semilattice_metadata_t &metadata, signal_t *interruptor);
