...
http_res_t distribution_app_t::handle(const http_req_t &req) {
    ...
    if (!maybe_n_id || str_to_uuid(*maybe_n_id).is_nil()) {
        http_res_t res(400);
        res.set_body("application/text", "Valid uuid required for query paramter \"namespace\"\n");
        return res;
    }
    ...
    if (maybe_depth) {
        if (!strtou64_strict(maybe_depth.get(), 10, &depth) || depth == 0 || depth > MAX_DETPH) {
            http_res_t res(400);
    ...
    namespace_repo_t<memcached_protocol_t>::namepsace_if_access_t ns_access = ns_repo->get_namespace_if(n_id);

    memcached_protocol_t::read_t read(distribution_get_query_t(depth), time(NULL));
    cond_t interrupt;
    memcached_protocol_t::read_response_t db_res = ns_access.namespace_if->read(read, order_token_t::ignore, &interrupt);
