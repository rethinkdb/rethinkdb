...
typename json_adapter_if_t<ctx_t>::json_adapter_map_t get_json_subfields(database_semilattice_metadata_t *target, const ctx_t &) {
...
typename json_adapter_if_t<ctx_t>::json_adapter_map_t get_json_subfields(databases_semilattice_metadata_t *target, const ctx_t &ctx) {
    return json_adapter_with_inserter_t<databases_semilattice_metadata_t::database_map_t, ctx_t>(&target->databases, generate_uuid).get_subfields(ctx);
}
