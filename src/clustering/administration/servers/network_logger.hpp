...
public:
    network_logger_t(
            ...
            const boost::shared_ptr<semilattice_read_view_t<servers_semilattice_metadata_t> > &semilattice_view);
...
private:
    ...
    peer_id_t us;
    clone_ptr_t<watchable_t<change_tracking_map_t<peer_id_t, cluster_directory_metadata_t> > > directory_view;
    boost::shared_ptr<semilattice_read_view_t<servers_semilattice_metadata_t> > semilattice_view;

    watchable_t<change_tracking_map_t<peer_id_t, cluster_directory_metadata_t> >::subscription_t directory_subscription;
    ...
    std::set<server_id_t> connected_servers;
