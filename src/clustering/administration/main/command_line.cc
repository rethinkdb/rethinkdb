...
options::help_section_t get_network_options(const bool join_required, std::vector<options::option_t> *options_out) {
    ...
    help.add("--cluster-reconnect-timeout seconds", "maximum number of seconds to attempt reconnecting to a node before giving up");
...
int main_rethinkdb_serve(int argc, char *argv[]) {
    ...
    try {
        ...
        boost::optional<int> node_reconnect_timeout_secs = parse_node_reconnect_timeout_secs_option(opts);
        ...
        serve_info_t serve_info(std::move(joins),
                                ...
                                node_reconnect_timeout_secs ?
                                    node_reconnect_timeout_secs.get() : cluster_defaults::reconnect_timeout,
                                tls_configs);
