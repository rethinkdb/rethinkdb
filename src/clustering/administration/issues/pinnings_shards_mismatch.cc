...
std::string pinnings_shards_mismatch_issue_t<protocol_t>::get_description() const {
    ...
    return strprintf("The namespace: %s has a pinning map which is segmented differently than its sharding scheme.\n"
                      ...
                      scoped_cJSON_t(render_as_json(&_shards, 0)).PrintUnformatted().c_str(),
                      scoped_cJSON_t(render_as_json(&_primary_pinnings, 0)).PrintUnformatted().c_str(),
                      scoped_cJSON_t(render_as_json(&_secondary_pinnings, 0)).PrintUnformatted().c_str());
}
