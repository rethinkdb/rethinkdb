    ...
    void file_away(const std::map<uuid_u, deletable_t<object_metadata_t> > &map) {
        for (typename std::map<uuid_u, deletable_t<object_metadata_t> >::const_iterator it = map.begin();
            ...
            if (!it->second.is_deleted()) {
                by_name[it->second.get_ref().name.get_ref()].insert(it->first);
            }
        }
    }
...
public:
    ...
    void file_away(const std::map<uuid_u, deletable_t<object_metadata_t> > &map) {
        for (typename std::map<uuid_u, deletable_t<object_metadata_t> >::const_iterator it = map.begin();
                it != map.end(); it++) {
            if (!it->second.is_deleted()) {
                if (std_contains(databases, it->second.get_ref().database.get_ref())) {
                    deletable_t<database_semilattice_metadata_t> db =
                        databases[it->second.get_ref().database.get_ref()];
                    if (!db.is_deleted()) {
                        by_name[db_table_name_t(db.get_ref().name.get_ref(),
...
std::list<clone_ptr_t<global_issue_t> > name_conflict_issue_tracker_t::get_issues() {
    ...
    name_map_t servers;
    servers.file_away(metadata.machines.machines);
    servers.report("server", &issues);
