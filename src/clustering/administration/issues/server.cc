...
ql::datum_t build_server_issue_info(
        ...
        const std::vector<server_id_t> &affected_server_ids) {
...
server_issue_tracker_t::server_issue_tracker_t(
        ...
        server_to_peer(_server_to_peer),
        server_to_peer_subs(std::bind(&server_issue_tracker_t::recompute, this)) {
    watchable_t<change_tracking_map_t<peer_id_t, server_id_t> >::freeze_t
        freeze(server_to_peer);
    server_to_peer_subs.reset(server_to_peer, &freeze);
    recompute();   
}
...
void server_issue_tracker_t::recompute() {
    std::vector<server_id_t> down_servers;
    std::vector<server_id_t> ghost_servers;

    cluster_semilattice_metadata_t metadata = cluster_sl_view->get();
    for (auto const &server : metadata.servers.servers) {
        peer_id_t peer = server_id_to_peer_id(server.first,
                                              server_to_peer->get().get_inner());
        if (!server.second.is_deleted() && peer.is_nil()) {
            down_servers.push_back(server.first);
        } else if (server.second.is_deleted() && !peer.is_nil()) {
            ghost_servers.push_back(server.first);
        }
    }
...
void server_issue_tracker_t::combine(
    ...
    {
        std::map<server_id_t, server_ghost_issue_t*> combined_ghost_issues;
