...
bool convert_type_and_id_from_datum(ql::datum_t primary_key,
                                    std::string *type_out,
                                    uuid_u *id_out) {
    std::string error;
...
bool jobs_artificial_table_backend_t::read_row(ql::datum_t primary_key,
    ...
    *row_out = ql::datum_t();
    ...
    if (convert_type_and_id_from_datum(primary_key, &job_report_type, &job_report_id)) {
        std::map<uuid_u, job_report_t> job_reports;
        get_all_job_reports(&ct_interruptor, &job_reports);
...
bool jobs_artificial_table_backend_t::write_row(ql::datum_t primary_key,
    ...
    if (new_value_inout->has()) {
        error_out->assign("You may only delete from the `rethinkdb.jobs` table.");
        return false;
    }
    ...
    std::string type;
    uuid_u id;
    if (convert_type_and_id_from_datum(primary_key, &type, &id)) {
        if (type != "query") {
            error_out->assign("You can currently only interrupt `\"query\"` jobs.");
            return false;
        }
