...
template <class protocol_t>
std::map<peer_id_t, typename namespace_repo_t<protocol_t>::master_map_t> get_master_map(const std::map<peer_id_t, namespaces_directory_metadata_t<protocol_t> > &ns_directory_metadata, namespace_id_t n_id) {
    std::map<peer_id_t, typename namespace_repo_t<protocol_t>::master_map_t> res;
    for (typename std::map<peer_id_t, namespaces_directory_metadata_t<protocol_t> >::const_iterator it  = ns_directory_metadata.begin();
        ...
        if (std_contains(it->second.master_maps, n_id)) {
            ...
            res[it->first] = typename namespace_repo_t<protocol_t>::master_map_t();
        }
    }
...
template <class protocol_t>
typename namespace_repo_t<protocol_t>::namepsace_if_access_t namespace_repo_t<protocol_t>::get_namespace_if(namespace_id_t n_id) {
