...
static store_key_t interpolate_key(store_key_t in1, store_key_t in2, double fraction) {
    ...
    store_key_t out(min(i + num_interp, MAX_KEY_SIZE), out_buf);
...
static void ensure_distinct(std::vector<store_key_t> *split_points) {
    for (size_t i = 1; i < split_points->size(); ++i) {
        ...
        guarantee(split_points[i] >= split_points[i-1]);
    }
    for (size_t i = 1; i < split_points->size(); ++i) {
        /* Normally, we fix any overlaps by just pushing keys forward. */
        while (split_points[i] <= split_points[i-1]) {
            bool ok = split_points[i].increment();
            if (!ok) {
                /* Oops, we ran into the maximum possible key. This is incredibly
                unlikely in practice, but we handle it anyway. */
                size_t j = i;
                while (j > 1 && split_points[j] == split_points[j-1]) {
                    bool ok2 = split_points[j-1].decrement();
                    rassert(ok2);
                    --j;
                }
            }
...
bool calculate_split_points_with_distribution(
    ...
    for (size_t split_index = 1; split_index < num_shards; ++split_index) {
        int64_t split_count = (split * total) / num_shards;
...
void calculate_split_points_by_estimation(
    ...
    if (num_shards == old_split_points.size() + 1) {
    ...
    for (size_t split_index = 1; split_index < num_shards; ++split_index) {
        ...
        guarantee(left_old_index >= 0 && left_old_index <= old_split_points.size());
        store_key_t left_key = (left_old_index == 0)
            ? store_key_t::min()
            : old_split_points[left_old_index-1];
        store_key_t right_key =(left_old_index+1 == old_split_points.size())
            ? store_key_t::max()
