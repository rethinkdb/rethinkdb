...
bool db_config_artificial_table_backend_t::write_row(
    ...
    if (new_value.has()) {
        /* We're creating a new database or updating an existing one */
        ...
        /* Determine whether we're creating a new database or updating an existing one */
        ...
        if (it != md.databases.end() && !it->second.is_deleted()) {
            ...
            guarantee(it == md.databases.end(), "UUID collision happened");
        }

        if (is_creation || new_db_name != old_db_name) {
