...
shard_status_t calculate_shard_status(
    ...
    [&](const region_t &, const region_acks_t &region_acks) {
        ...
        for (const auto &ack : region_acks.acks) {
            auto &server_status = shard_status.replicas[ack.first];
            switch (ack.second.second.state) {
                case contract_ack_t::state_t::primary_need_branch:
                    has_unfinished = true;
                    server_status = std::max(
                        server_status, server_status_t::WAITING_FOR_QUORUM);
                    break;
                case contract_ack_t::state_t::secondary_need_primary:
                    region_has_outdated_reader = true;
                    has_unfinished = true;
                    server_status = std::max(
                        server_status, server_status_t::WAITING_FOR_PRIMARY);
                    break;
                ...
                case contract_ack_t::state_t::primary_ready:
                    ...
                    server_status = std::max(server_status, server_status_t::READY);
                    break;
                case contract_ack_t::state_t::secondary_backfilling:
                    has_unfinished = true;
                    server_status = std::max(
                        server_status, server_status_t::BACKFILLING);
                    break;
                case contract_ack_t::state_t::secondary_streaming:
                    {
                        ...
                        if (static_cast<bool>(latest_contract.primary) &&
                            ...
                            region_has_outdated_reader = true;
                            server_status = std::max(
                                server_status, server_status_t::READY);
                        } else {
                            has_unfinished = true;
                            server_status = std::max(
                                server_status, server_status_t::TRANSITIONING);
                        }
                    }
                    break;
                case contract_ack_t::state_t::nothing:
                    server_status = std::max(server_status, server_status_t::NOTHING);
                    break;
            }
        }
        ...
        for (const auto &replica : replicas) {
            if (shard_status.replicas.find(replica) == shard_status.replicas.end()) {
                has_unfinished = true;
                auto &server_status = shard_status.replicas[replica];
                server_status = std::max(
                    server_status,
