...
typename json_adapter_if_t<ctx_t>::json_adapter_map_t get_json_subfields(persistable_blueprint_t<protocol_t> *target, const ctx_t &ctx) {
    typename json_adapter_if_t<ctx_t>::json_adapter_map_t res;
    res["peers_roles"] = boost::shared_ptr<json_adapter_if_t<ctx_t> >(new json_adapter_t<typename persistable_blueprint_t<protocol_t>::role_map_t, ctx_t>(&target->machines_roles, ctx));
    return  res;
}
