...
public:
    ...
    namespaces_semilattice_metadata_t<mock::dummy_protocol_t> namespaces;
    machines_semilattice_metadata_t machines;


    RDB_MAKE_ME_SERIALIZABLE_2(namespaces, machines);
};
...
typename json_adapter_if_t<ctx_t>::json_adapter_map_t get_json_subfields(cluster_semilattice_metadata_t *target, const ctx_t &) {
    ...
    res["namespaces"] = boost::shared_ptr<json_adapter_if_t<ctx_t> >(new json_adapter_t<namespaces_semilattice_metadata_t<mock::dummy_protocol_t>, ctx_t>(&target->namespaces));
