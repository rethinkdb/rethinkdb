...
ql::datum_t convert_timespec_to_datum(const timespec &t) {
    return ql::pseudo::make_time(
        t.seconds + static_cast<double>(t.nanoseconds) / 1e9, "+00:00");
}
...
bool convert_timespec_from_datum(
    ...
    double epoch_time = time_to_epoch_time(d);
    double ipart, fpart;
    fpart = modf(epoch_time, &ipart);
    t_out->seconds = ipart;
    t_out->nanoseconds = fpart * 1e9;
    return true;
}
...
ql::datum_t convert_log_message_to_datum(
    ...
    builder.add("id", convert_log_key_to_datum(msg.timestamp, server_id));
    builder.add("server", server_datum);
    builder.add("timestamp", convert_timespec_to_datum(msg.timespec));
    builder.add("uptime", convert_timespec_to_datum(msg.uptime));
    builder.add("level", ql::datum_t(datum_string_t(format_log_level(msg.level))));
    builder.add("message", ql::datum_t(datum_string_t(msg.message)));
...
bool logs_artificial_table_backend_t::read_all_rows_as_vector(
    ...
    directory->read_all(
        [&](const peer_id_t &key, const cluster_directory_metadata_t *value) {
...
bool logs_artificial_table_backend_t::read_row(
    ...
    ql::datum_t server_datum;
    name_string_t server_name;
    if (!convert_server_id_to_datum(server_id, name_client,
            &server_datum, &server_name, &dummy_error)) {
        /* The server doesn't exist in the metadata */
        *row_out = ql::datum_t();
        return true;
    }

    boost::optional<peer_id_t> peer_id = name_client->get_peer_id_for_server_id(server);
