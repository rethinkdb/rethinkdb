...
cluster_reql_admin_interface_t::cluster_reql_admin_interface_t(
        uuid_u mmi,
        boost::shared_ptr<semilattice_readwrite_view_t<cluster_semilattice_metadata_t> > s,
        clone_ptr_t<watchable_t<change_tracking_map_t<peer_id_t, cluster_directory_metadata_t> > > d
        ) :
    my_machine_id(mmi),
    semilattices(s),
    directory(d),
    ...
    switch (status) {
        case METADATA_SUCCESS: {
            if (expect_present) {
                ...
                *error_out = strprintf("There is already a %s called `%s`.", entity_type, entity_name.c_str());
                return false;
            }
        }
        case METADATA_ERR_MULTIPLE: {
            if (expect_present) {
                *error_out = strprintf("There are multiple %ss called `%s`.", entity_type, entity_name.c_str());
            } else {
                *error_out = strprintf("There is already a %s called `%s`.", entity_type, entity_name.c_str());
            }
            return false;
        ...
        case METADATA_CONFLICT: {
            *error_out = "There is a metadata conflict; please resolve it before trying to create a database.";
            return false;
        }
        case METADATA_ERR_NONE: {
            if (expect_present) {
                *error_out = strprintf("Could not find any %s called `%s`.", entity_type, entity_name.c_str());
                return false;
            } else {
                return true;
            }
...
bool cluster_reql_admin_interface_t::db_create(const name_string_t &name,
    ...
    cluster_semilattice_metadata_t metadata;
    {
        ...
        metadata_searcher_t<database_semilattice_metadata_t> db_searcher(&metadata.databases.databases);

        metadata_search_status_t status;
        db_searcher.find_uniq(name, &status);
        if (!check_metadata_status(status, "database", name.str(), false, error_out)) return false;
...
bool cluster_reql_admin_interface_t::db_drop(const name_string_t &name,
    ...
    cluster_semilattice_metadata_t metadata;
    {
        ...
        if (!check_metadata_status(status, "database", name.str(), true, error_out)) return false;
        ...
        cow_ptr_t<namespaces_semilattice_metadata_t>::change_t ns_change(&metadata.rdb_namespaces);
        metadata_searcher_t<namespace_semilattice_metadata_t> ns_searcher(&ns_change.get()->namespaces);
...
bool cluster_reql_admin_interface_t::db_list(
    ...
    const_metadata_searcher_t<database_semilattice_metadata_t> db_searcher(&db_metadata.databases);
    for (auto it = db_searcher.find_next(db_searcher.begin()); it != db_searcher.end(); it = db_searcher.find_next(++it)) {
...
bool cluster_reql_admin_interface_t::table_create(const name_string_t &name, counted_t<const ql::db_t> db,
    ...
    cluster_semilattice_metadata_t metadata;
    {
        ...
        uuid_u dc_id;
        if (primary_dc) {
            metadata_searcher_t<datacenter_semilattice_metadata_t> dc_searcher(&metadata.datacenters.datacenters);
            metadata_search_status_t status;
            auto it = dc_searcher.find_uniq(*primary_dc, &status);
            if (!check_metadata_status(status, "datacenter", primary_dc->str(), true, error_out)) return false;
            dc_id = it->first;
        } else {
            dc_id = nil_uuid();
        }
        ...
        /* Make sure there isn't an existing table with the same name */
        {
            ...
            if (!check_metadata_status(status, "table", db->name + "." + name.str(), false, error_out)) return false;
        }

        /* Construct a description of the new namespace */
        namespace_semilattice_metadata_t table = new_namespace(my_machine_id, db->id, dc_id, name, primary_key);
        std::map<datacenter_id_t, ack_expectation_t> *ack_map = &table.ack_expectations.get_mutable();
        ...
        *namespace_id_out = generate_uuid();
        ns_change.get()->namespaces.insert(std::make_pair(*namespace_id_out, make_deletable(table)));

        try {
            fill_in_blueprints(&metadata, directory->get().get_inner(), my_machine_id, boost::optional<namespace_id_t>());
        } catch (const missing_machine_exc_t &exc) {
            *error_out = strprintf("Could not configure table: %s Table was not created.", exc.what());
            return false;
        }
...
bool cluster_reql_admin_interface_t::table_drop(const name_string_t &name, counted_t<const ql::db_t> db,
        signal_t *interruptor, std::string *error_out) {
    cluster_semilattice_metadata_t metadata;
    {
        on_thread_t thread_switcher(semilattices->home_thread());
        ...
        if (!check_metadata_status(status, "table", db->name + "." + name.str(), true, error_out)) return false;
...
bool cluster_reql_admin_interface_t::table_list(counted_t<const ql::db_t> db,
    ...
    const_metadata_searcher_t<namespace_semilattice_metadata_t> ns_searcher(&ns_metadata->namespaces);
    ...
    for (auto it = ns_searcher.find_next(ns_searcher.begin(), pred);
        ...
        guarantee(!it->second.is_deleted());
        if (it->second.get_ref().name.in_conflict()) {
            /* Maybe we should raise an error instead of silently ignoring the table with the conflicted name */
            continue;
        }
        names_out->insert(it->second.get_ref().name.get());
    }
...
bool cluster_reql_admin_interface_t::table_find(const name_string_t &name, counted_t<const ql::db_t> db,
        UNUSED signal_t *interruptor, uuid_u *namespace_id_out, std::string *primary_key_out, std::string *error_out) {
...
void cluster_reql_admin_interface_t::wait_for_metadata_to_propagate(const cluster_semilattice_metadata_t &metadata, signal_t *interruptor) {
...
cow_ptr_t<namespaces_semilattice_metadata_t> cluster_reql_admin_interface_t::get_namespaces_metadata() {
    int threadnum = get_thread_id().threadnum;
    r_sanity_check(cross_thread_namespace_watchables[threadnum].has());
...
void cluster_reql_admin_interface_t::get_databases_metadata(databases_semilattice_metadata_t *out) {
    int threadnum = get_thread_id().threadnum;
    r_sanity_check(cross_thread_database_watchables[threadnum].has());
