...
private:
    void on_read(typename protocol_t::read_t read, order_token_t otok,
            address_t<void(boost::variant<typename protocol_t::read_response_t, std::string>)> response_address,
    ...
    void on_write(typename protocol_t::write_t write, order_token_t otok,
            address_t<void(boost::variant<typename protocol_t::write_response_t, std::string>)> response_address,
            auto_drainer_t::lock_t keepalive)
            THROWS_NOTHING
    {
        keepalive.assert_is_holding(&drainer);
