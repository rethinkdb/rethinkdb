...
bool backfiller_t<protocol_t>::confirm_and_send_metainfo(typename store_view_t<protocol_t>::metainfo_t metainfo, DEBUG_VAR region_map_t<protocol_t, version_range_t> start_point,
    ...
    guarantee_reviewed(metainfo.get_domain() == start_point.get_domain());
        ...
        for (typename version_map_t::const_iterator it = start_point.begin();
            ...
            for (typename version_map_t::const_iterator jt = end_point.begin();
                ...
                if (!region_is_empty(ixn)) {
                    ...
                    rassert_reviewed(start.timestamp <= end.timestamp);
                    rassert_reviewed(version_is_ancestor(branch_history_manager, start, end, ixn));
                }
            }
        }
    }
...
void backfiller_t<protocol_t>::on_backfill(backfill_session_id_t session_id,
    ...
    assert_thread();
    guarantee_reviewed(region_is_superset(svs->get_region(), start_point.get_domain()));
