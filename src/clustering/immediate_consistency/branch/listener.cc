...
public:
    ...
    void check_metainfo(const region_map_t<protocol_t, binary_blob_t>& metainfo, const typename protocol_t::region_t& region) const {
        ...
        for (typename region_map_t<protocol_t, binary_blob_t>::const_iterator it = masked.begin(); it != masked.end(); ++it) {
            ...
            rassert_unreviewed(range.earliest.timestamp == range.latest.timestamp);
            rassert_unreviewed(range.latest.timestamp <= tstamp_);
        }
    }
        ...
        mailbox_callback_mode_inline)
{
    ...
    guarantee_unreviewed(region_is_superset(our_branch_region_, svs_->get_region()));
...

    {
        ...
        for (typename region_map_t<protocol_t, version_range_t>::const_iterator it = start_point.begin();
            ...
            rassert_unreviewed(version.branch == branch_id_ ||
    ...
    guarantee_unreviewed(registration_done_cond_.get_ready_signal()->is_pulsed());
    ...
    guarantee_unreviewed(backfill_end_point.begin() != backfill_end_point.end());
    ...
    for (typename region_map_t<protocol_t, version_range_t>::const_iterator it = backfill_end_point.begin();
         ...
         ++it) {
        rassert_unreviewed(it->second.is_coherent());
        rassert_unreviewed(it->second.earliest.branch == branch_id_);
        backfill_end_timestamp = std::max(backfill_end_timestamp, it->second.earliest.timestamp);
    }

    guarantee_unreviewed(backfill_end_timestamp >= streaming_begin_point);
        ...
        mailbox_callback_mode_inline)
{
    ...
    boost::optional<boost::optional<broadcaster_business_card_t<protocol_t> > > business_card =
        broadcaster_metadata->get();
    rassert_unreviewed(business_card && business_card.get());
    rassert_unreviewed(business_card.get().get().branch_id == broadcaster->get_branch_id());
    ...
    rassert_unreviewed(svs_->get_region() == this_branch_history.region);
    ...
    rassert_unreviewed(expected_initial_metainfo == initial_metainfo);
#endif
...
public:
    ...
    void fill(listener_intro_t<protocol_t> _intro) {
        guarantee_unreviewed(!is_pulsed());
        intro = _intro;
        pulse();
    }
};
        ...
        THROWS_ONLY(interrupted_exc_t, broadcaster_lost_exc_t)
{
    ...
    if (registrant_->get_failed_signal()->is_pulsed()) {
        ...
        guarantee_unreviewed(intro_receiver.is_pulsed());
...
void listener_t<protocol_t>::on_write(const typename protocol_t::write_t &write,
    ...
    guarantee_unreviewed(region_is_superset(our_branch_region_, write.get_region()));
    guarantee_unreviewed(!region_is_empty(write.get_region()));
...
void listener_t<protocol_t>::on_writeread(const typename protocol_t::write_t &write,
        ...
        THROWS_NOTHING
{
    rassert_unreviewed(region_is_superset(our_branch_region_, write.get_region()));
    rassert_unreviewed(!region_is_empty(write.get_region()));
    rassert_unreviewed(region_is_superset(svs_->get_region(), write.get_region()));
    order_token.assert_write_mode();
...
void listener_t<protocol_t>::on_read(const typename protocol_t::read_t &read,
        ...
        THROWS_NOTHING
{
    rassert_unreviewed(region_is_superset(our_branch_region_, read.get_region()));
    rassert_unreviewed(!region_is_empty(read.get_region()));
    rassert_unreviewed(region_is_superset(svs_->get_region(), read.get_region()));
        ...
        auto_drainer_t::lock_t keepalive) THROWS_NOTHING
{
    try {
        object_buffer_t<fifo_enforcer_sink_t::exit_read_t> read_token;
        {
            ...
            rassert_unreviewed(current_timestamp_ == expected_timestamp);
...
void listener_t<protocol_t>::advance_current_timestamp_and_pulse_waiters(transition_timestamp_t timestamp) {
    rassert_unreviewed(timestamp.timestamp_before() == current_timestamp_);
    ...
    for (std::multimap<state_timestamp_t, cond_t *>::const_iterator it = synchronize_waiters_.begin();
        ...
        if (it->first < current_timestamp_) {
            rassert_unreviewed(it->second->is_pulsed(), "This cond should have already been pulsed because we assume timestamps move in discrete minimal steps.");
        } else {
