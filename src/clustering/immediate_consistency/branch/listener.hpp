...
private:
    ...
    void on_writeread(auto_drainer_t::lock_t keepalive,
            ...
            transition_timestamp_t transition_timestamp,
            fifo_enforcer_write_token_t fifo_token,
            typename mailbox_t<void(typename protocol_t::write_response_t)>::address_t ack_addr)
            THROWS_NOTHING
    {
        try {
            boost::scoped_ptr<fifo_enforcer_sink_t::exit_write_t> token;
    ...
    void on_read(auto_drainer_t::lock_t keepalive,
            ...
            typename mailbox_t<void(typename protocol_t::read_response_t)>::address_t ack_addr)
            THROWS_NOTHING
    {
        try {
