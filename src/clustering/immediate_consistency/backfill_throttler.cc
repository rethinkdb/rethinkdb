...
const int64_t GLOBAL_LIMIT = 16;
const int64_t PER_PEER_LIMIT = 4;
...
backfill_throttler_t::lock_t::lock_t(backfill_throttler_t *p,
    ...
    cross_thread_signal_t ct_interruptor(interruptor, parent->home_thread());
    {
        ...
        std::map<peer_id_t, scoped_ptr_t<new_semaphore_t> >::iterator peer_sem_it;
        {
            ...
            if (peer_sem_it == parent->peer_sems.end()) {
                peer_sem_it = parent->peer_sems.insert(
                        std::make_pair(from_peer,
                                       make_scoped<new_semaphore_t>(PER_PEER_LIMIT))).first;
            }
        ...
        try {
            ...
            peer_acq.init(new new_semaphore_acq_t(peer_sem_it->second.get(), 1));
