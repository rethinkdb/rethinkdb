...
typename json_adapter_if_t<ctx_t>::json_adapter_map_t get_json_subfields(blueprint_t<protocol_t> *target, const ctx_t &) {
    typename json_adapter_if_t<ctx_t>::json_adapter_map_t res;
    res["peers_roles"] = boost::shared_ptr<json_adapter_if_t<ctx_t> >(new json_adapter_t<typename blueprint_t<protocol_t>::role_map_t, ctx_t>(&target->peers_roles));
    return  res;
}

template <class protocol_t, class ctx_t>
cJSON *render_as_json(blueprint_t<protocol_t> *target, const ctx_t &ctx) {
    return render_as_directory(target, ctx);
}

template <class protocol_t, class ctx_t>
void apply_json_to(cJSON *change, blueprint_t<protocol_t> *target, const ctx_t &ctx) {
