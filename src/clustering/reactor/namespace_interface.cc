    ...
    THROWS_NOTHING
{
    ...
    rassert_reviewed(region_is_superset(master_to_contact->region, sharded_op.get_region()));
    rassert_reviewed(region_is_superset(operation->get_region(), sharded_op.get_region()));
    ...
    try {
        ...
        guarantee_reviewed(interruptor->is_pulsed());
    ...
    THROWS_NOTHING
{
    ...
    rassert_reviewed(region_is_superset(direct_reader_to_contact->region, sharded_op.get_region()));
    rassert_reviewed(region_is_superset(operation->get_region(), sharded_op.get_region()));
...
void cluster_namespace_interface_t<protocol_t>::relationship_coroutine(peer_id_t peer_id, reactor_activity_id_t activity_id,
    ...
    try {
        ...
        if (is_start) {
            guarantee_reviewed(start_count > 0);
            start_count--;
            if (start_count == 0) {
                start_cond.pulse();
            }
