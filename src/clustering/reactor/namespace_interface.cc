    ...
    THROWS_NOTHING
{
    ...
    try {
        ...
        mailbox_t<void(typename protocol_t::read_response_t)> cont(mailbox_manager,
                                                                   std::bind(&outdated_read_store_result<protocol_t>, &results->at(i), _1, &done));
    ...
    try {
        ...
        if (is_primary) {
            master_access.init(new master_access_t<protocol_t>(mailbox_manager,
                                                               directory_view->subview(std::bind(&cluster_namespace_interface_t<protocol_t>::extract_master_business_card, _1, peer_id, activity_id)),
                                                               lock.get_drain_signal()));
            direct_reader_access.init(new resource_access_t<direct_reader_business_card_t<protocol_t> >(directory_view->subview(std::bind(&cluster_namespace_interface_t<protocol_t>::extract_direct_reader_business_card_from_primary, _1, peer_id, activity_id))));
        } else {
            direct_reader_access.init(new resource_access_t<direct_reader_business_card_t<protocol_t> >(directory_view->subview(std::bind(&cluster_namespace_interface_t<protocol_t>::extract_direct_reader_business_card_from_secondary, _1, peer_id, activity_id))));
        }
