...
json_adapter_if_t::json_adapter_map_t get_json_subfields(backfill_location_t *target, const ctx_t &ctx) {
    ...
    res["backfill_session_id"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<backfill_session_id_t, ctx_t>(&target->backfill_session_id, ctx));
    res["peer_id"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<peer_id_t, ctx_t>(&target->peer_id, ctx));
    res["activity_id"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<reactor_activity_id_t, ctx_t>(&target->activity_id, ctx));
    return res;
}
...
json_adapter_if_t::json_adapter_map_t get_json_subfields(primary_when_safe_t<protocol_t> *target, const ctx_t &ctx) {
    ...
    res["backfillers"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<std::vector<backfill_location_t>, ctx_t>(&target->backfills_waited_on, ctx));
    return res;
}
...
json_adapter_if_t::json_adapter_map_t get_json_subfields(primary_t<protocol_t> *target, const ctx_t &ctx) {
    ...
    //res["broadcaster"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<broadcaster_business_card_t<protocol_t>, ctx_t>(&target->broadcaster));
    //res["replier"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<boost::optional<replier_business_card_t<protocol_t> >, ctx_t>(&target->replier));
    return res;
}
...
json_adapter_if_t::json_adapter_map_t get_json_subfields(secondary_up_to_date_t<protocol_t> *target, const ctx_t &ctx) {
    ...
    res["branch_id"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<branch_id_t, ctx_t>(&target->branch_id, ctx));
    // TODO: git blame this and ask why it's commented out.
    //res["replier"] =   boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<replier_business_card_t<protocol_t>, ctx_t>(&target->replier));
    return res;
}
...
json_adapter_if_t::json_adapter_map_t get_json_subfields(secondary_without_primary_t<protocol_t> *, const ctx_t &ctx) {
    ...
    // TODO: git blame this and ask why it's commented out.
    //res["current_state"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<region_map_t<protocol_t, version_range_t>, ctx_t>(&target->current_state));
    //res["backfiller"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<backfiller_business_card_t<protocol_t>, ctx_t>(&target->backfiller));
    return res;
}
...
json_adapter_if_t::json_adapter_map_t get_json_subfields(secondary_backfilling_t<protocol_t> *target, const ctx_t &ctx) {
    ...
    res["backfiller"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<backfill_location_t, ctx_t>(&target->backfill, ctx));
    return res;
}
...
json_adapter_if_t::json_adapter_map_t get_json_subfields(reactor_business_card_t<protocol_t> *target, const ctx_t &ctx) {
    json_adapter_if_t::json_adapter_map_t res;
    res["activity_map"] = boost::shared_ptr<json_adapter_if_t>(new json_adapter_t<typename reactor_business_card_t<protocol_t>::activity_map_t, ctx_t>(&target->activities, ctx));
    return res;
}
