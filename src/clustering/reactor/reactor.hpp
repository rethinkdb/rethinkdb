...
public:
    reactor_t(
            ...
            clone_ptr_t<watchable_t<std::map<peer_id_t, boost::optional<directory_echo_wrapper_t<boost::shared_ptr<const reactor_business_card_t<protocol_t> > > > > > > reactor_directory,
    ...
    clone_ptr_t<watchable_t<directory_echo_wrapper_t<boost::shared_ptr<const reactor_business_card_t<protocol_t> > > > > get_reactor_directory() {
...
private:
    ...
    bool is_safe_for_us_to_be_primary(const std::map<peer_id_t, boost::shared_ptr<const reactor_business_card_t<protocol_t> > > &reactor_directory, const blueprint_t<protocol_t> &blueprint,
    ...
    bool find_broadcaster_in_directory(const typename protocol_t::region_t &region, const blueprint_t<protocol_t> &bp, const std::map<peer_id_t, boost::shared_ptr<const reactor_business_card_t<protocol_t> > > &reactor_directory,
                                       clone_ptr_t<watchable_t<boost::optional<boost::optional<broadcaster_business_card_t<protocol_t> > > > > *broadcaster_out);

    bool find_replier_in_directory(const typename protocol_t::region_t &region, const branch_id_t &b_id, const blueprint_t<protocol_t> &bp, const std::map<peer_id_t, boost::shared_ptr<const reactor_business_card_t<protocol_t> > > &reactor_directory,
    ...
    bool is_safe_for_us_to_be_nothing(const std::map<peer_id_t, boost::shared_ptr<const reactor_business_card_t<protocol_t> > > &reactor_directory, const blueprint_t<protocol_t> &blueprint,
    ...
    directory_echo_writer_t<boost::shared_ptr<const reactor_business_card_t<protocol_t> > > directory_echo_writer;
    directory_echo_mirror_t<boost::shared_ptr<const reactor_business_card_t<protocol_t> > > directory_echo_mirror;
    branch_history_manager_t<protocol_t> *branch_history_manager;
...
boost::optional<boost::optional<activity_t> > extract_activity_from_reactor_bcard(const std::map<peer_id_t, boost::shared_ptr<const reactor_business_card_t<protocol_t> > > &bcards, peer_id_t p_id, const reactor_activity_id_t &ra_id) {
    typename std::map<peer_id_t, boost::shared_ptr<const reactor_business_card_t<protocol_t> > >::const_iterator it = bcards.find(p_id);
    if (it == bcards.end()) {
