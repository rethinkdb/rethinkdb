...
void reactor_t::update_best_backfiller(const region_map_t<rdb_protocol_t, version_range_t> &offered_backfill_versions,
                                       const backfill_candidate_t::backfill_location_t &backfiller,
                                       best_backfiller_map_t *best_backfiller_out) {
    for (region_map_t<rdb_protocol_t, version_range_t>::const_iterator i =  offered_backfill_versions.begin();
...
boost::optional<boost::optional<backfiller_business_card_t<rdb_protocol_t> > > extract_backfiller_from_reactor_business_card_secondary(
    ...
    if (!bcard) {
        ...
        return boost::optional<boost::optional<backfiller_business_card_t<rdb_protocol_t> > >(
            boost::optional<backfiller_business_card_t<rdb_protocol_t> >());
    } else {
        return boost::optional<boost::optional<backfiller_business_card_t<rdb_protocol_t> > >(
            boost::optional<backfiller_business_card_t<rdb_protocol_t> >(
            ...
            if (!region_is_empty(intersection)) {
                ...
                try {
                    if (const rb_t::secondary_without_primary_t * secondary_without_primary = boost::get<rb_t::secondary_without_primary_t>(&it->second.activity)) {
                        branch_history_t<rdb_protocol_t> bh = secondary_without_primary->branch_history;
                        std::set<branch_id_t> known_branchs = branch_history_manager->known_branches();

                        for (std::map<branch_id_t, branch_birth_certificate_t<rdb_protocol_t> >::iterator bt  = bh.branches.begin();
                        ...
                        branch_history_t<rdb_protocol_t> bh = nothing_when_safe->branch_history;
                        std::set<branch_id_t> known_branchs = branch_history_manager->known_branches();
...
void do_backfill(
    ...
    try {
        cross_thread_watchable_variable_t<boost::optional<boost::optional<backfiller_business_card_t<rdb_protocol_t> > > > ct_backfiller_metadata(backfiller_metadata, svs->home_thread());
        cross_thread_signal_t ct_interruptor(interruptor, svs->home_thread());
        on_thread_t th(svs->home_thread());

        backfillee<rdb_protocol_t>(mailbox_manager, branch_history_manager, svs, region, ct_backfiller_metadata.get_watchable(), backfill_session_id, &ct_interruptor);
...
template <class protocol_t>
bool check_that_we_see_our_broadcaster(const boost::optional<boost::optional<broadcaster_business_card_t<rdb_protocol_t> > > &maybe_a_business_card) {
...
bool reactor_t::attempt_backfill_from_peers(directory_entry_t *directory_entry,
    ...
    region_map_t<rdb_protocol_t, version_range_t> metainfo = to_version_range_map(metainfo_blob);
    region_map_t<rdb_protocol_t, backfill_candidate_t> best_backfillers = region_map_transform<rdb_protocol_t, version_range_t, backfill_candidate_t>(metainfo, &reactor_t::make_backfill_candidate_from_version_range);
    ...
    while (true) {
        branch_history_t<rdb_protocol_t> branch_history_to_merge;
...
void reactor_t::be_primary(region_t region, store_view_t<rdb_protocol_t> *svs, const clone_ptr_t<watchable_t<blueprint_t<rdb_protocol_t> > > &blueprint, signal_t *interruptor) THROWS_NOTHING {
    try {
        ...
        broadcaster_t<rdb_protocol_t> broadcaster(mailbox_manager, branch_history_manager, svs, &region_perfmon_collection, &order_source, &ct_interruptor);
        ...
        cross_thread_watchable_variable_t<boost::optional<boost::optional<broadcaster_business_card_t<rdb_protocol_t> > > > ct_broadcaster_business_card(broadcaster_business_card, svs->home_thread());

        on_thread_t th3(svs->home_thread());
        listener_t<rdb_protocol_t> listener(base_path, io_backender, mailbox_manager, ct_broadcaster_business_card.get_watchable(), branch_history_manager, &broadcaster, &region_perfmon_collection, &ct_interruptor, &order_source);
        replier_t<rdb_protocol_t> replier(&listener, mailbox_manager, branch_history_manager);
        master_t<rdb_protocol_t> master(mailbox_manager, ack_checker, region, &broadcaster);
