...
void reactor_t<protocol_t>::run_cpu_sharded_role(
    ...
    case blueprint_role_primary:
        be_primary(region, svs_subview->get_store(cpu_shard_number), role->blueprint.get_watchable(), interruptor);
        break;
    case blueprint_role_secondary:
        be_secondary(region, svs_subview->get_store(cpu_shard_number), role->blueprint.get_watchable(), interruptor);
        break;
    case blueprint_role_nothing:
        be_nothing(region, svs_subview->get_store(cpu_shard_number), role->blueprint.get_watchable(), interruptor);
        break;
    default:
        unreachable();
        break;
