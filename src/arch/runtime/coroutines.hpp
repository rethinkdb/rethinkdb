...
to call `coro_t::wait()`, `coro_t::spawn_now_deprecated()`, or `coro_t::notify_now()`
...
within that block may call `coro_t::spawn_now_deprecated()` or `coro_t::notify_now()` but
not `coro_t::wait()`. This is because `coro_t::spawn_now_deprecated()` and
