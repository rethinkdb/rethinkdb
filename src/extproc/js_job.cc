...
js_result_t js_job_t::eval(const std::string &source) {
    ...
    archive_result_t res
        = deserialize<cluster_version_t::LATEST_OVERALL>(extproc_job.read_stream(),
                                                         &result);
    if (bad(res)) {
        throw js_worker_exc_t(strprintf("failed to deserialize eval result from worker "
...
js_result_t js_job_t::call(js_id_t id, const std::vector<counted_t<const ql::datum_t> > &args) {
    ...
    archive_result_t res
        = deserialize<cluster_version_t::LATEST_OVERALL>(extproc_job.read_stream(),
                                                         &result);
    if (bad(res)) {
        throw js_worker_exc_t(strprintf("failed to deserialize call result from worker "
                                        "(%s)", archive_result_as_str(res)));
    }
    return result;
}
...
void js_job_t::release(js_id_t id) {
    ...
    serialize<cluster_version_t::LATEST_OVERALL>(&wm, id);
    {
        int res = send_write_message(extproc_job.write_stream(), &wm);
        if (res != 0) {
            throw js_worker_exc_t("failed to send data to the worker");
        }
    }
    ...
    if (bad(res) || !dummy_result) {
        throw js_worker_exc_t(strprintf("failed to deserialize release result from worker "
                                        "(%s)", archive_result_as_str(res)));
    }
}
...
void js_job_t::exit() {
    ...
    // dummy_result should always be true
    if (bad(res) || !dummy_result) {
        throw js_worker_exc_t(strprintf("failed to deserialize exit result from worker "
                                        "(%s)", archive_result_as_str(res)));
    }
}
