...
js_result_t js_job_t::eval(const std::string &source) {
    ...
    serialize<cluster_version_t::LATEST_VERSION>(&wm, source);
    {
...
js_result_t js_job_t::call(js_id_t id, const std::vector<counted_t<const ql::datum_t> > &args) {
    ...
    write_message_t wm;
    wm.append(&task, sizeof(task));
    serialize<cluster_version_t::LATEST_VERSION>(&wm, id);
    serialize<cluster_version_t::LATEST_VERSION>(&wm, args);
    {
        int res = send_write_message(extproc_job.write_stream(), &wm);
        if (res != 0) { throw js_worker_exc_t("failed to send data to the worker"); }
    }
        ...
        case TASK_EVAL:
            {
                ...
                write_message_t wm;
                serialize<cluster_version_t::LATEST_VERSION>(&wm, js_result);
        ...
        case TASK_CALL:
            {
                ...
                {
                    archive_result_t res
                        = deserialize<cluster_version_t::LATEST_VERSION>(stream_in, &id);
                    if (bad(res)) { return false; }
                    res = deserialize<cluster_version_t::LATEST_VERSION>(stream_in, &args);
                    if (bad(res)) { return false; }
                }
