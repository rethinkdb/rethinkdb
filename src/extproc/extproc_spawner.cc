...
fd_t extproc_spawner_t::spawn() {
    ...
    int send_res = send_fds(spawner_socket.get(), 1, &fds[1]);
    guarantee_err(send_res == 0, "could not send socket file descriptor to worker process");
