...
NORETURN bool worker_exit_fn(read_stream_t *stream_in, write_stream_t *) {
    int exit_code;
    archive_result_t res = deserialize<cluster_version_t::LATEST_VERSION>(stream_in,
                                                                          &exit_code);
...
extproc_worker_t::~extproc_worker_t() {
    if (worker_pid != -1) {
        ...
        int exit_code = 0;
        write_message_t wm;
        serialize<cluster_version_t::LATEST_VERSION>(&wm, exit_code);
...
void extproc_worker_t::released(bool user_error, signal_t *user_interruptor) {
    ...
    if (user_interruptor != NULL && user_interruptor->is_pulsed()) {
        ...
        try {
            write_message_t wm;
            serialize<cluster_version_t::LATEST_VERSION>(&wm, parent_to_worker_magic);
            {
            ...
            archive_result_t res
                = deserialize<cluster_version_t::LATEST_VERSION>(socket_stream.get(),
                                                                 &magic_from_child);
