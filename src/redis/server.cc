    ...
    next_thread(0)
{
    ...
    std::vector<redis_protocol_t::store_t *> stores;

    for (int i = 0; i < (int)shards.size(); i++) {

        serializer_t *serializer = multiplexer->proxies[i];

        redis_protocol_t::store_t::create(serializer, shards[i]);

        redis_protocol_t::store_t *store =
            new redis_protocol_t::store_t(serializer, shards[i]);
