    ...
    public home_thread_mixin_t
{
    ...
    bool install() {
        ...
        waiting_for_shutdown[get_thread_id()] = false;
        return true;
    }
    ...
    bool shutting_down;
    // Have a separate shutting-down flag per thread so that we don't get spurious calls to
    // have_uninstalled() generated by log messages that end after we call shutdown() but before
    // uninstall() reaches the thread in question
    bool waiting_for_shutdown[MAX_THREADS];
    int num_threads_up;
    ...
    bool uninstall() {
        ...
        log_controller = NULL;
        waiting_for_shutdown[get_thread_id()] = true;
    ...
    bool done(char *msg) {
        free(msg);
        messages_out[get_thread_id()]--;
        if (waiting_for_shutdown[get_thread_id()] && messages_out[get_thread_id()] == 0) {
            do_on_thread(home_thread, this, &log_controller_t::have_uninstalled);
        }
        return true;
    }
