...
http_res_t routing_http_app_t::handle(const http_req_t &req, signal_t *interruptor) {
    ...
    if (it == req.resource.end() || !std_contains(subroutes, *it)) {
        ...
        if (defaultroute) {
            return defaultroute->handle(req, interruptor);
        } else {
        ...
        std::string route(*it);
        ++it;
        return subroutes[route]->handle(http_req_t(req, it), interruptor);
    }
}
