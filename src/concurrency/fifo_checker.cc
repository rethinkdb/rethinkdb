...
void order_sink_t::verify_token_value_and_update(order_token_t token, std::pair<tagged_seen_t, tagged_seen_t> *ls_pair) {
    ...
    if (token.read_mode_) {
        rassert(token.value_ >= ls_pair->first.value, "read_mode expected (0x%llx >= 0x%llx), (%s >= %s), bucket = (%s)", (long long unsigned int)token.value_, (long long unsigned int)ls_pair->first.value, token.tag_.c_str(), ls_pair->first.tag.c_str(), uuid_to_str(token.bucket_.uuid_).c_str());
        if (ls_pair->second.value < token.value_) {
            ls_pair->second = tagged_seen_t(token.value_, token.tag_);
        }
    } else {
        rassert(token.value_ >= ls_pair->second.value, "write_mode expected (0x%llx >= 0x%llx), (%s >= %s), bucket = (%s)", (long long unsigned int)token.value_, (long long unsigned int)ls_pair->second.value, token.tag_.c_str(), ls_pair->second.tag.c_str(), uuid_to_str(token.bucket_.uuid_).c_str());
