...
on the producer to ask for data, rather than the producer calling a method on the
consumer to give it data.
...
protected:
    /* To implement a passive producer: Pass a `watchable_t<bool>*` to the
    constructor; the current value of this `watchable_t<bool>` will be taken
    to indicate whether data is available. Override `produce_next_value()`;
    `produce_next_value()` will only be called when the `watchable_t<bool>` is
    true, but it must never fail or block. */
    passive_producer_t(watchable_t<bool> *w) : available(w) { }
