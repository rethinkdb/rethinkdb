...
public:
    ...
    void index_read(block_id_t block_id, refc_ptr_t<ls_block_token_pointee_t> *tok_out);

    void block_read(const refc_ptr_t<ls_block_token_pointee_t>& token, void *buf, file_account_t *io_account, iocallback_t *cb);

    void block_read(const refc_ptr_t<ls_block_token_pointee_t>& token, void *buf, file_account_t *io_account);

    void index_write(const std::vector<index_write_op_t>& write_ops, file_account_t *io_account);

    void block_write(const void *buf, block_id_t block_id, file_account_t *io_account, iocallback_t *cb, refc_ptr_t<ls_block_token_pointee_t> *tok_out);
    void block_write(const void *buf, file_account_t *io_account, iocallback_t *cb, refc_ptr_t<ls_block_token_pointee_t> *tok_out);
    void block_write(const void *buf, block_id_t block_id, file_account_t *io_account, refc_ptr_t<ls_block_token_pointee_t> *tok_out);
    void block_write(const void *buf, file_account_t *io_account, refc_ptr_t<ls_block_token_pointee_t> *tok_out);
...
private:
    ...
    void generate_block_token(off64_t offset, refc_ptr_t<ls_block_token_pointee_t> *tok_out);

    std::vector<serializer_read_ahead_callback_t*> read_ahead_callbacks;
    bool offer_buf_to_read_ahead_callbacks(block_id_t block_id, void *buf, const refc_ptr_t<standard_block_token_t>& token, repli_timestamp_t recency_timestamp);
