    ...
    public alloc_mixin_t<tls_small_obj_alloc_accessor<alloc_t>, lba_start_fsm_t >
{
    ...
    lba_start_fsm_t(lba_list_t *l)
        ...
        // TODO: Allocation
        superblock_buffer = (lba_superblock_t*)malloc_aligned(owner->extent_manager->extent_size, DEVICE_BLOCK_SIZE);
        assert(superblock_buffer);
    }
        ...
        if (state == state_load_superblock) {
            ...
            if (owner->lba_superblock_offset == OFFSET_UNINITIALIZED) {
                ...
                queue->iosys.schedule_aio_read(
                    ...
                    (lba_list_t::lba_superblock_t::entry_count_to_file_size(owner->lba_superblock_entry_count)
                     / DEVICE_BLOCK_SIZE * DEVICE_BLOCK_SIZE), // (this takes care of alignment)
                    superblock_buffer,
                    queue,
                    this);
    ...
    lba_superblock_t *superblock_buffer;
...
void lba_list_t::prepare_metablock(metablock_mixin_t *metablock) {
    ...
    if (current_extent) {
        ...
        metablock->last_lba_extent_offset = last_extent_offset;
        metablock->last_lba_extent_entries_count = last_extent_entry_count;
    }
}
