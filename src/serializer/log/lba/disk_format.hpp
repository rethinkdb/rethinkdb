...
struct lba_superblock_t {
    ...
    // Returns false this operation would overflow or if nentries is
    // not positive.  nentries could still be an obviously invalid
    // value (like if file_size_out was greater than a possible
    // superblock size).
