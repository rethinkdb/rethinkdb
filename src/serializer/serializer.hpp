...
struct index_write_op_t {
    index_write_op_t(block_id_t _block_id,
                     ...
                     const refc_ptr_t<standard_block_token_t>& token = refc_ptr_t<standard_block_token_t>(),
		     boost::optional<repli_timestamp_t> _recency = boost::none)
	: block_id(_block_id), modify_buf(_modify_buf), buf_token(), recency(_recency) {
        buf_token.copy_from(token);
    }

    void make_modify_buf(const refc_ptr_t<standard_block_token_t>& token) {
        modify_buf = true;
        buf_token.copy_from(token);
    }

    bool wants_modify_buf(refc_ptr_t<standard_block_token_t> *buf_token_out) const {
        if (modify_buf) {
            buf_token_out->copy_from(buf_token);
            return true;
        }
        return false;
    }

    index_write_op_t(const index_write_op_t& x) : block_id(x.block_id), modify_buf(x.modify_buf), buf_token(), recency(x.recency) {
        buf_token.copy_from(x.buf_token);
    }

    void operator=(const index_write_op_t& x) {
        block_id = x.block_id;
        modify_buf = x.modify_buf;
        buf_token.copy_from(x.buf_token);
        recency = x.recency;
    }
...
private:
    ...
    bool modify_buf;
    refc_ptr_t<standard_block_token_t> buf_token;
public:
    ...
    public home_thread_mixin_t
{
    ...
    */
    ...
    virtual void block_read(const refc_ptr_t<standard_block_token_t>& token, void *buf, file_account_t *io_account, iocallback_t *cb) = 0;

    // Blocking variant (requires coroutine context). Has default implementation.
    virtual void block_read(const refc_ptr_t<standard_block_token_t>& token, void *buf, file_account_t *io_account) = 0;
    ...
    virtual void index_read(block_id_t block_id, refc_ptr_t<standard_block_token_t> *tok_out) = 0;
    ...
    virtual void block_write(const void *buf, block_id_t block_id, file_account_t *io_account, iocallback_t *cb, refc_ptr_t<standard_block_token_t> *tok_out) = 0;
    ...
    virtual void block_write(const void *buf, file_account_t *io_account, iocallback_t *cb, refc_ptr_t<standard_block_token_t> *tok_out);
    ...
    virtual void block_write(const void *buf, file_account_t *io_account, refc_ptr_t<standard_block_token_t> *tok_out);
    virtual void block_write(const void *buf, block_id_t block_id, file_account_t *io_account, refc_ptr_t<standard_block_token_t> *tok_out);
...
struct serializer_write_launched_callback_t {
    virtual void on_write_launched(const refc_ptr_t<standard_block_token_t>& token) = 0;
...
template <class serializer_type>
void serializer_block_write(serializer_type *ser, const void *buf, file_account_t *io_account, iocallback_t *cb, refc_ptr_t<typename serializer_traits_t<serializer_type>::block_token_type> *tok_out) {
    ser->block_write(buf, NULL_BLOCK_ID, io_account, cb, tok_out);
}

// Blocking variants.
template <class serializer_type>
void serializer_block_write(serializer_type *ser, const void *buf, file_account_t *io_account, refc_ptr_t<typename serializer_traits_t<serializer_type>::block_token_type> *tok_out) {
...
template <class serializer_type>
void serializer_block_write(serializer_type *ser, const void *buf, block_id_t block_id, file_account_t *io_account, refc_ptr_t<typename serializer_traits_t<serializer_type>::block_token_type> *tok_out) {
    struct : public cond_t, public iocallback_t {
        void on_io_complete() { pulse(); }
    } cb;
