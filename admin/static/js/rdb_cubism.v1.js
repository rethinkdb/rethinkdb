...
// Slava's sensible chart version
cubism_contextPrototype.sensible = function() {
  var context = this,
      width = context.size(),
      height = 120,
      scale = d3.scale.linear().interpolate(d3.interpolateRound),
      ...
      extent = null,
      title = cubism_identity,
      formatPrimary = cubism_sensiblePrimaryFormat,
      formatChange = cubism_sensibleChangeFormat,
      colors = ["#9ecae1", "#225b84", "#a1d99b", "#22723a"],
      strokeWidth = 1.5;

  function sensible(selection) {

    selection
        .on("mousemove.sensible", function() { context.focus(d3.mouse(this)[0]); })
        .on("mouseout.sensible", function() { context.focus(null); });
    ...
    selection.each(function(d, i) {
      ...
      // Note that someone still needs to listen to the metric,
      // so that it continues to update automatically.
      primary_.on("change.sensible-" + id, firstChange);
      secondary_.on("change.sensible-" + id, firstChange);
      function firstChange(start, stop) {
        change(start, stop), focus();
        if (ready) {
          primary_.on("change.sensible-" + id, cubism_identity);
          secondary_.on("change.sensible-" + id, cubism_identity);
        }
      }

      // Update the chart when the context changes.
      context.on("change.sensible-" + id, change);
      context.on("focus.sensible-" + id, focus);
    });
  }

  sensible.remove = function(selection) {

    selection
        .on("mousemove.sensible", null)
        .on("mouseout.sensible", null);
    ...
    function remove(d) {
      d.primary.on("change.sensible-" + d.id, null);
      d.secondary.on("change.sensible-" + d.id, null);
      context.on("change.sensible-" + d.id, null);
      context.on("focus.sensible-" + d.id, null);
    }
  };

  sensible.height = function(_) {
    if (!arguments.length) return height;
    height = +_;
    return sensible;
  };

  sensible.primary = function(_) {
    if (!arguments.length) return primary;
    primary = _;
    return sensible;
  };

  sensible.secondary = function(_) {
    if (!arguments.length) return secondary;
    secondary = _;
    return sensible;
  };

  sensible.scale = function(_) {
    if (!arguments.length) return scale;
    scale = _;
    return sensible;
  };

  sensible.extent = function(_) {
    if (!arguments.length) return extent;
    extent = _;
    return sensible;
  };

  sensible.title = function(_) {
    if (!arguments.length) return title;
    title = _;
    return sensible;
  };

  sensible.formatPrimary = function(_) {
    if (!arguments.length) return formatPrimary;
    formatPrimary = _;
    return sensible;
  };

  sensible.formatChange = function(_) {
    if (!arguments.length) return formatChange;
    formatChange = _;
    return sensible;
  };

  sensible.colors = function(_) {
    if (!arguments.length) return colors;
    colors = _;
    return sensible;
  };

  sensible.strokeWidth = function(_) {
    if (!arguments.length) return strokeWidth;
    strokeWidth = _;
    return sensible;
  };
...
var cubism_sensiblePrimaryFormat = d3.format(".2s"),
